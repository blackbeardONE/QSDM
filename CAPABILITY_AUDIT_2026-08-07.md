...
Two remaining items are **operator actions, not code**: choosing a coordinated `fork_dust_height` (and re-deriving genesis first), and publishing npm `qsdm-sdk@0.3.1`.
...
| 13 | Go/npm SDK distribution | **FIXED (npm publish outstanding)** | **Go:** the SDK was not `go get`-able because the parent module declares `github.com/blackbeardONE/QSDM` while its go.mod sits at `QSDM/source/` — a path that does not match its location, so nothing under it resolves. Gave the SDK its own module (`QSDM/source/sdk/go/go.mod`), which is self-contained: it depends only on stdlib and nothing in the parent imports it. `go get github.com/blackbeardONE/QSDM/QSDM/source/sdk/go` now works; install path documented in the package doc and Feature Summary; added a CI step since a separate module falls outside `./...`. **npm:** the 0.3.1 source is already correct — `getTransaction` hits the registered `/api/v1/transactions/{id}` route — only the publish itself is outstanding, which needs registry credentials and is an outward-facing action. Feature Summary now states plainly that 0.3.0 is what is published. |
...
- npm `qsdm-sdk` is stuck at 0.3.0 with a permanently-404ing `getTransaction`; the 0.3.1 publish run failed (27145442319) and was never retried.
