...
func TestGatewayOptionalHiveConsumerAPI(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"streams list", http.MethodGet, "/api/v1/streams", http.StatusAccepted},
		{"stream action nonce", http.MethodGet, "/api/v1/streams/nonce", http.StatusAccepted},
		{"stream detail", http.MethodGet, "/api/v1/streams/stream-1", http.StatusAccepted},
		...
		{"signed stream action", http.MethodPost, "/api/v1/streams/actions/submit-signed", http.StatusAccepted},
		{"path traversal blocked", http.MethodGet, "/api/v1/tasks/../admin/accounts", http.StatusForbidden},
		{"stream path traversal blocked", http.MethodGet, "/api/v1/streams/../admin/accounts", http.StatusForbidden},
