 ...
 *
 *   const { QSDMPlusClient, ApiError, isNotFound, isUnauthorized } = require('qsdmplus');
 *   const c = new QSDMPlusClient('http://node:8080');
 *   c.setToken(jwt);
...
class ApiError extends Error {
    constructor(status, url, bodyText) {
        super(`qsdmplus: ${url} returned ${status}: ${truncate(bodyText, 256)}`);
        this.name = 'ApiError';
        this.status = status;
        this.url = url;
        this.body = bodyText;
...
class QSDMPlusClient {
    ...
    constructor(baseURL, opts = {}) {
        if (typeof baseURL !== 'string' || baseURL.length === 0) {
            throw new Error('QSDMPlusClient: baseURL is required');
        }
        ...
        if (!this._fetch) {
            throw new Error('QSDMPlusClient: fetch is not available; pass opts.fetch');
        }
    ...
    async getNodeStatus() {
        ...
        return {
            ...
            branding:   raw.branding && typeof raw.branding === 'object' ? {
                ...
                legacyName: raw.branding.legacy_name,
    ...
    async _request(method, path, body) {
        ...
        try {
            ...
            throw new Error(`qsdmplus: failed to decode JSON from ${url}: ${e.message}`);
        }
    }
...
if (typeof module !== 'undefined' && module.exports) {
    ...
    // Back-compat alias.
    module.exports.QSDMClient = QSDMPlusClient;
}
