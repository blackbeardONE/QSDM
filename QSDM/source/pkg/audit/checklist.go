...
func defaultItems() []ChecklistItem {
	return []ChecklistItem{
		...
		{ID: "api-06", Category: CatAPI, Severity: SevHigh, Title: "Self-custody signed-transaction submission (v0.4 design — POST /api/v1/wallet/submit-signed)", Description: "DESIGN PHASE (Session 94, not yet implemented — see QSDM/docs/docs/V040_WALLET_SEND_DESIGN.md). Once implemented, the handler must (a) decode the wallet.TransactionData envelope and enforce sender == hex(sha256(public_key)) so a caller cannot claim to be an arbitrary address whose key they don't hold, (b) verify the ML-DSA-87 signature over the canonical payload (envelope JSON with `signature` and `public_key` fields removed, json.Marshal default ordering) using the envelope's own public_key, NEVER falling back to a validator-side keypair, (c) consult storage.GetBalance(sender) before applying the debit (HTTP 402 on insufficient funds), (d) idempotent on tx_id (HTTP 409 + echo on duplicate), (e) bump qsdm_wallet_send_total{result=success|signature_invalid|insufficient_balance|duplicate|store_failed|...} for every terminal path. The existing /api/v1/wallet/send is NOT a substitute — it uses the validator's own wallet (pkg/wallet/wallet.go::CreateTransaction always sets Sender=ws.address, ignoring JWT claims). Known v0.4.0 gaps tracked in the design doc: (1) no per-account nonce → replay possible across distinct tx_ids; (2) pkg/storage/sqlite.go::UpdateBalance warns-and-proceeds on negative balance instead of rolling back atomically. Both must close before `mining-05` (incentivised testnet) exposure."},
