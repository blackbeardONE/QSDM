...
func defaultItems() []ChecklistItem {
	return []ChecklistItem{
		...
		{ID: "runtime-07", Category: CatRuntime, Severity: SevMedium, Title: "NetworkPolicy / egress control", Description: "Default-deny NetworkPolicy restricts pod egress to known Scylla / metrics / P2P peers only.", Status: StatusPassed, ReviewedBy: "evidence:in-tree", ReviewedAt: ts("2026-05-14T19:45:00Z"), Notes: "QSDM/deploy/kubernetes/networkpolicy.yaml ships 8 NetworkPolicy objects: (1) qsdm-default-deny — empty Ingress+Egress rules selecting all pods, (2) qsdm-allow-dns-egress — kube-system kube-dns UDP+TCP/53, (3) qsdm-allow-intra-namespace — same-namespace TCP/4001 + TCP/8080 for BFT consensus + miner→validator proof submission, (4) qsdm-allow-scylla-egress — namespace 'scylla' TCP/9042+9142 (CQL + CQL+TLS), (5) qsdm-allow-prometheus-scrape — namespace 'monitoring' ingress to TCP/8080+8081, (6) qsdm-allow-ingress-controller — namespace 'ingress-nginx' ingress to TCP/8080+8081, (7) qsdm-allow-libp2p-egress — internet egress TCP/4001 with private-CIDR exclusions, (8) qsdm-allow-ntp-egress — internet egress UDP/123 for wall-clock attestation freshness. Validated by .github/workflows/validate-deploy.yml — 8/8 NetworkPolicy resources kubeconform Valid against k8s 1.31."},
