...
// alias qsdmplus_node_id) with value (after trim) equal to expectedNodeID (after trim), so bundles
// can be bound to a specific node.
// If proofHMACSecret is non-empty, qsdm_proof_hmac (or legacy qsdmplus_proof_hmac) must be a valid
...
func NvidiaLockProofOK(maxAge time.Duration, expectedNodeID, proofHMACSecret string, consumeMatching bool) (ok bool, detail string) {
	...
	if len(ngcProofs) == 0 {
		return false, "NVIDIA lock: no NGC proof bundles ingested; run the NGC sidecar with QSDM_NGC_REPORT_URL (legacy QSDMPLUS_NGC_REPORT_URL still accepted) and matching ingest secret"
	}
	...
	if wantID != "" {
		return false, "NVIDIA lock: no qualifying proof within window with matching qsdm_node_id / qsdmplus_node_id (set QSDM_NGC_PROOF_NODE_ID on the sidecar to match QSDM_NVIDIA_LOCK_EXPECTED_NODE_ID on the node; QSDMPLUS_* names remain accepted, plus GPU attestation as usual)"
	}
	if strings.TrimSpace(proofHMACSecret) != "" {
		return false, "NVIDIA lock: no qualifying proof with valid qsdm_proof_hmac / qsdmplus_proof_hmac (set QSDM_NGC_PROOF_HMAC_SECRET on the sidecar to match QSDM_NVIDIA_LOCK_PROOF_HMAC_SECRET on the node; QSDMPLUS_* names remain accepted)"
	}
	return false, "NVIDIA lock: no qualifying proof within window (need GPU-attested bundle: architecture mentions NVIDIA and gpu_fingerprint.available=true); use GPU profile sidecar or widen QSDM_NVIDIA_LOCK_MAX_PROOF_AGE (legacy QSDMPLUS_NVIDIA_LOCK_MAX_PROOF_AGE still accepted)"
}
