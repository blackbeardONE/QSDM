...
install a local validator. Version 1.3.91 bundles the native `qsdmcli` signer,
...
task state. Hive 1.3.91 isolates that route behind a bounded outage circuit.
...
Hive 1.3.91 adds an **Application Compute Gateway** to the Mother Hive task. While the task and its paired Relay are online, native applications can submit fixed CPU, NVIDIA GPU, or RAM jobs through the authenticated loopback endpoint at `http://127.0.0.1:7742`. Hive shows its status and private credential path on the Mother Hive page. The gateway queues work durably on the Relay, returns verified results and receipts, and never accepts uploaded code, scripts, commands, or executables. Unmodified applications cannot see the pool as local hardware; they must use this API or the packaged `qsdm-edge-agent compute` commands. See the [pooled edge-compute guide](EDGE_POOL.md#application-compute-gateway) for examples.
