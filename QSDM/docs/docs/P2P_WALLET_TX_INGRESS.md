...
This note is for operators running `qsdmplus` with libp2p pubsub wallet traffic and optional mesh wire.
...
- When `QSDMPLUS_PUBLISH_MESH_COMPANION` is `1` / `true` / `yes`, the API (and the demo wallet loop after a successful gossip publish) may send a **second** pubsub frame: a `qsdm_mesh3d_v1` envelope whose payload is the **same** wallet JSON bytes.
...
- **In-process:** `pkg/walletp2p` (`Reserve` / `Release` / `NoteIngested`) tracks recent wallet JSON `id` values so duplicate ingress is dropped before expensive validation. `cmd/qsdmplus/transaction` uses `Reserve` on legacy pubsub JSON and mesh inner JSON; **`pkg/networking.TxGossipIngress` calls `NoteIngested` when gossip validation accepts**, so the same id is not processed again on the legacy wallet/mesh path. Metric: `qsdmplus_p2p_wallet_ingress_dedupe_skip_total`.
...
| Variable | Purpose |
|----------|---------|
| `QSDMPLUS_PUBLISH_MESH_COMPANION` | Also publish mesh wire after wallet JSON (API + demo loop). |
| `QSDMPLUS_WASM_PREFLIGHT_MODULE` | Path to `wasm_module.wasm` for wazero `validate_raw` preflight on P2P JSON. |
