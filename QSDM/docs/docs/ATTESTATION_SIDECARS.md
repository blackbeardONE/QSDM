...
The node aggregates distinct sources by the `qsdm_node_id` field
...
| # | Host | Role | Shape / OS | `QSDM_NGC_PROOF_NODE_ID` |
|---|------|------|------------|------------------------------|
| 1 | Windows dev PC | GPU attestation, Scheduled Task every 10 min | RTX 3050, CC 8.6 | `qsdm-windows-dev` |
| 2 | DigitalOcean BLR1 | CPU-fallback sidecar, `systemd` timer every 10 min | Ubuntu 22.04 | `qsdm-vps` |
| 3 | Oracle Cloud ap-singapore-1 | CPU-fallback sidecar, `systemd` timer every 10 min | E5.Flex (billable) or A1.Flex (free) | `qsdm-oci` |
...
2. **Share the same `QSDM_NGC_INGEST_SECRET`.** Set once on the
...
`apps/qsdm-nvidia-ngc/scripts/attest-from-env-file.ps1`; a Scheduled
Task invokes it every 10 min. The env file lives beside it as
`ngc.local.env` (gitignored) with:

```
QSDM_NGC_REPORT_URL=https://api.qsdm.tech/api/v1/monitoring/ngc-proof
QSDM_NGC_INGEST_SECRET=<same secret as the BLR1 node>
...
$action = New-ScheduledTaskAction `
    ...
    -Argument '-NoProfile -ExecutionPolicy Bypass -File ' +
              '"E:\Projects\QSDM\apps\qsdm-nvidia-ngc\scripts\attest-from-env-file.ps1" ' +
              '-EnvFile "E:\Projects\QSDM\apps\qsdm-nvidia-ngc\ngc.local.env" -Quiet'
...
The script reads the running `QSDM_NGC_INGEST_SECRET` out of the
...
```bash
$env:QSDM_NGC_INGEST_SECRET = '<same secret as BLR1>'
...
|---|---|---|
| `attested` stays at `1` after adding a sidecar | Duplicate `QSDM_NGC_PROOF_NODE_ID` | Set a distinct id in the new sidecar's `ngc.env`; restart its timer. |
| `qsdm_ngc_proof_ingest_rejected_total{reason="hmac_mismatch"}` climbs | Sidecar's `QSDM_NGC_INGEST_SECRET` drifted from node's | Re-sync from the node's systemd env; rotate everywhere at once. |
...
| Timer fires but no POST arrives | Sidecar cannot reach the ingest URL | `curl -v $QSDM_NGC_REPORT_URL` from the sidecar host; check Caddy/firewall. |
...
- Example alert rules (internal Alertmanager):
  `QSDM/deploy/prometheus/alerts_qsdm.example.yml` group
