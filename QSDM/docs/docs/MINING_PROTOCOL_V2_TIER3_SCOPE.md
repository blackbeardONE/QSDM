...
- v2 mining CLI: `cmd/qsdmcli/mining.go` + `cmd/qsdmcli/mining_test.go`. Subcommands `enroll`, `unenroll`, `slash`, `enrollment-status`, `slash-receipt` cover the full v2 HTTP surface. Builds canonical payloads through `pkg/mining/{enrollment,slashing}` so the CLI shares the exact codec the mempool admission gate validates against — no parallel hand-rolled JSON path. Unit tests assert envelope shape, base64 round-trip, missing-flag rejection, hex-decode failure surfacing, HTTP path escaping, and 4xx propagation across all five subcommands.
