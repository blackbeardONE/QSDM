...
- Slash-helper offline evidence assembly: `cmd/qsdmcli/slash_helper.go` + `cmd/qsdmcli/slash_helper_test.go`. Three subcommands (`forged-attestation`, `double-mining`, `inspect`) produce / decode the canonical `EvidenceBlob` bytes the chain-side `forgedattest` / `doublemining` decoders consume, so a watcher-bot operator no longer has to import the Go packages or hand-roll JSON to ship a slash. Pre-flight checks (proof version, NodeID binding, equal `(Epoch, Height)`, distinct canonical bytes) reject obviously-broken evidence locally rather than spending a tx fee on a guaranteed `verifier_failed`. Output defaults to stdout (raw bytes, ready to pipe into `qsdmcli slash --evidence-file=-`); `--out=PATH` writes to a 0o600 file; `--print-cmd` emits a copy-pasteable `qsdmcli slash …` snippet on stderr (never stdout, so it doesn't corrupt the bytes pipe). Encoder ordering is canonicalised in `double-mining` so two slashers observing the same equivocation produce byte-identical evidence — preserving chain-side per-fingerprint replay protection. 21 test cases cover happy paths, stdout-write, stderr-only `--print-cmd`, every pre-flight rejection, encoder round-trip, and `inspect` against forged-attestation + double-mining blobs supplied as either `--evidence-file` or `--evidence-hex`.
