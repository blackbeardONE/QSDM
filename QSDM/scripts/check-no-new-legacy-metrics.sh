...
#      branding aliases like `qsdmplus_node_id` in
...
ALLOWLIST=(
  ...
  # producer).
  "QSDM/source/sdk/javascript/qsdmplus.test.js"
  ...
  "QSDM/deploy/prometheus/alerts_qsdmplus.example.yml"
  # Prometheus deploy README documents that "series names use the
  # qsdmplus_ prefix". Keeping this in place until the legacy prefix
  ...
  # filename is `qsdmplus-overview.json`, it was imported by operators
  ...
  "QSDM/deploy/grafana/qsdmplus-overview.json"
  ...
  # `qsdmplus_ngc_proof_ingest_accepted_total`"). Operators running
  ...
  "apps/qsdmplus-nvidia-ngc/QUICKSTART.md"
...
# to flag non-metric identifiers like `qsdmplus_node_id` in the NGC
