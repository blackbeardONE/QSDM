# Example: fetch dashboard /api/metrics/prometheus and list qsdmplus_submesh_* lines.
# Usage:
...
$lines = ($text -split "`r?`n") | Where-Object { $_ -match '^qsdmplus_submesh_' }
if (-not $lines) {
    Write-Error "No qsdmplus_submesh_* lines (JWT or METRICS_SECRET may be required)."
    exit 1
}
