...
validator: login is `root`, the `qsdmplus` service is already running
locally, and the installer can pull `QSDMPLUS_NGC_INGEST_SECRET` out
...
attestation source like an OCI Always-Free / E-series VM, which:
  ...
  * does NOT run `qsdmplus` locally — the sidecar's only job is to
...
/opt/qsdmplus/ngc-sidecar, systemd oneshot + 10 min timer, one-shot
...
def main() -> int:
    ...
    p.add_argument("--node-id", required=True,
                   help="Free-form QSDMPLUS_NGC_PROOF_NODE_ID. MUST differ from every other sidecar.")
    ...
    try:
        print(f"=== 1. /opt/qsdmplus/ngc-sidecar (sudo) ===")
        ssh_run(c,
            "sudo mkdir -p /opt/qsdmplus/ngc-sidecar && "
            "sudo chmod 0750 /opt/qsdmplus/ngc-sidecar && "
            "sudo chown root:root /opt/qsdmplus/ngc-sidecar")
        ...
        ssh_run(c,
            ...
            "/tmp/validator_phase1.py /opt/qsdmplus/ngc-sidecar/validator_phase1.py && "
        ...
        sudo_write_file(c, "/opt/qsdmplus/ngc-sidecar/ngc.env", env_body, "0600")
        ssh_run(c, "sudo ls -la /opt/qsdmplus/ngc-sidecar/ngc.env")
