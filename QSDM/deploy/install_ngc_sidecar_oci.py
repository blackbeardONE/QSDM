...
from _deploy_host import DeploymentTargetError, ngc_proof_ingest_url, public_api_base
...
def main() -> int:
    ...
    p.add_argument("--report-url",
                   help="POST target override; default derives from the shared public QSDM API base.")
    p.add_argument("--public-api-base",
                   help="HTTPS public QSDM API origin; overrides config for the default report and verification URLs.")
    ...
    try:
        ...
        p.error(str(exc))
    report_url = args.report_url or ngc_proof_ingest_url(args.public_api_base)
    ...
    try:
        ...
        env_body = (
            ...
            f"{region_comment}"
            f"QSDM_NGC_REPORT_URL={report_url}\n"
        ...
        recent_url = f"{api_base}/api/v1/trust/attestations/recent?limit=10"
        print(ssh_run(c,
            f"curl -sS --max-time 10 "
            f"{shlex.quote(summary_url)}"))
        print()
        print(ssh_run(c,
            f"curl -sS --max-time 10 "
            f"{shlex.quote(recent_url)}"))
    finally:
        c.close()
