...
def _configured_target() -> str | None:
    ...
    value = payload.get("vps_ssh_target")
    if value is None:
        return None
    if not isinstance(value, str):
        raise DeploymentTargetError(
            f"Invalid QSDM endpoint configuration at {path}: vps_ssh_target must be a string."
        )
...
def target() -> DeploymentTarget:
    ...
    host_override = _nonempty_env("QSDM_VPS_HOST")
    raw_target = host_override or _nonempty_env("QSDM_RELEASE_SSH_TARGET") or _configured_target() or _DEFAULT_TARGET
    resolved = _parse_target(raw_target)

    user_override = _nonempty_env("QSDM_VPS_USER")
    port_override = _nonempty_env("QSDM_VPS_PORT")
    return DeploymentTarget(
        host=resolved.host,
        user=_validate_user(user_override) if user_override else resolved.user,
        port=_parse_port(port_override) if port_override else resolved.port,
    )
