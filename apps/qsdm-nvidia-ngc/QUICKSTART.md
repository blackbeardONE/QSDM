...
sent in the `X-QSDM-NGC-Secret` header. Generate one now and keep it:
...
On the host running `qsdm-validator` / `qsdm`, set the ingest secret and
...
```bash
sudo systemctl edit qsdm-validator    # or `qsdm` on pre-rebrand units
# Paste, save, close:
[Service]
Environment=QSDM_NGC_INGEST_SECRET=7f2c3c1d0b...ef9a

sudo systemctl restart qsdm-validator
```

**Docker (the `VALIDATOR_QUICKSTART.md` compose shape):** add
`-e QSDM_NGC_INGEST_SECRET=$NGC_INGEST_SECRET` to the `docker run`
...
curl -sS -o /dev/null -w '%{http_code}\n' \
  -H "X-QSDM-NGC-Secret: $NGC_INGEST_SECRET" \
...
```bash
curl -sS -H "X-QSDM-NGC-Secret: $NGC_INGEST_SECRET" \
  http://127.0.0.1:8080/api/v1/monitoring/ngc-proofs | jq '.[] | {
    node_id:      .qsdm_node_id,
...
curl -sS -H "Authorization: Bearer $SCRAPE_TOKEN" \
  ...
  | grep -E '(qsdm|qsdm)_ngc_proof_ingest_(accepted|rejected)_total'
```

`qsdm_ngc_proof_ingest_accepted_total` should be `>= 1`. During the
`qsdm_*` → `qsdm_*` dual-emit deprecation window (Major Update §6)
the legacy series `qsdm_ngc_proof_ingest_accepted_total` is emitted
...
Set `QSDM_NVIDIA_LOCK_EXPECTED_NODE_ID=my-validator-1` on the node,
and the same value as `QSDM_NGC_PROOF_NODE_ID=my-validator-1` on the
sidecar (inside `ngc.env`). Now every bundle includes
`qsdm_node_id: "my-validator-1"` and the node rejects any bundle that
...
|------------------------------------------|-------------------------------------------|--------------|
| `QSDM_NGC_PROOF_HMAC_SECRET=<x>`     | `QSDM_NVIDIA_LOCK_PROOF_HMAC_SECRET=<x>` | Each bundle is also HMAC'd with `<x>`; node rejects unsigned bundles. |
| `QSDM_NGC_FETCH_CHALLENGE=true`      | `nvidia_lock_require_ingest_nonce = true` (or equivalent env) | Sidecar pulls a fresh nonce per bundle → replay resistance. Node rate-limits the route to 15/min; sidecar honors `Retry-After`. |
| `QSDM_NGC_CHALLENGE_JITTER_MAX_SEC=8` | — | Random 0…8 s sleep before each challenge fetch; use when many validators share one egress NAT. |
...
- [ ] Rotate `QSDM_NGC_INGEST_SECRET` on a schedule (≤ 90 days
...
- [ ] Monitor `qsdm_ngc_proof_ingest_rejected_total{reason=...}` (legacy
      alias during the dual-emit window: `qsdm_ngc_proof_ingest_rejected_total`)
...
- [ ] Dashboard panel — import
      `QSDM/deploy/grafana/qsdm-overview.json`, which already charts the
...
1. Put the ingest secret in `apps/qsdm-nvidia-ngc/ngc.local.env`
...
3. Register the scheduled task (user-level, no admin required).
   ...
   $script  = "$repo\apps\qsdm-nvidia-ngc\scripts\attest-from-env-file.ps1"
   $envfile = "$repo\apps\qsdm-nvidia-ngc\ngc.local.env"
...
sudo systemctl edit qsdm-validator            # remove QSDM_NGC_INGEST_SECRET=
sudo systemctl restart qsdm-validator
```
...
|---|---|---|
| Sidecar logs `401 Unauthorized` on POST | Secret mismatch between sidecar and node | Re-export `QSDM_NGC_INGEST_SECRET` in both places; `docker compose up --force-recreate`. |
| Sidecar logs `429 Too Many Requests` on `/ngc-challenge` | Multiple validators behind one NAT hammering the route | Set `QSDM_NGC_CHALLENGE_JITTER_MAX_SEC=8` on each sidecar. |
| Sidecar gets `404` on POST | Node has `QSDM_NGC_INGEST_SECRET` unset | See §2 — ingest is off by default. |
...
| Node rejects bundle with `qsdm_node_id mismatch` | Sidecar's `QSDM_NGC_PROOF_NODE_ID` differs from node's `QSDM_NVIDIA_LOCK_EXPECTED_NODE_ID` | Align them in `ngc.env` and restart the sidecar only — no node restart needed. |
