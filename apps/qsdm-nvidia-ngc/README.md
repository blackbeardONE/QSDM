...
This folder implements **Phase 1–3 prototype** workloads described in `../../nvidia_locked_qsdm_blockchain_architecture.md`: deterministic CUDA-adjacent PoW simulation, AI/tensor proofs (PyTorch), replay-style computation hash, optional UDP gossip, and optional push of proof bundles into the main **QSDM** Go node.
...
From **`apps/qsdm-nvidia-ngc/`**, PowerShell:
...
```powershell
# Windows — from apps/qsdm-nvidia-ngc
...
2. Set the sidecar env (compose snippet or shell):
   ...
   - `QSDM_NGC_REPORT_URL` (or legacy `QSDM_NGC_REPORT_URL`) — e.g. `http://host.docker.internal:8080/api/v1/monitoring/ngc-proof`
   - `QSDM_NGC_INGEST_SECRET` (or legacy `QSDM_NGC_INGEST_SECRET`) — same value as the node
...
If the node has **NVIDIA-lock** enabled (`[api] nvidia_lock = true` or `QSDM_NVIDIA_LOCK=true`), mint/send/create-token APIs require a **recent** ingested proof with NVIDIA architecture and `gpu_fingerprint.available == true`. Use the **GPU** compose profile (`docker compose --profile gpu up`); the default CPU image’s fingerprint has `available: false` and does not satisfy the lock.

Optional **node binding:** set `QSDM_NVIDIA_LOCK_EXPECTED_NODE_ID` on the node and the same value in `QSDM_NGC_PROOF_NODE_ID` (legacy `QSDM_NGC_PROOF_NODE_ID`) on the sidecar so each bundle includes `qsdm_node_id` matching that node.

Optional **proof HMAC:** set `QSDM_NVIDIA_LOCK_PROOF_HMAC_SECRET` on the node (with NVIDIA-lock on) and the same value as `QSDM_NGC_PROOF_HMAC_SECRET` on the sidecar; bundles then include `qsdm_proof_hmac` (see deploy README).

**Ingest nonce:** when the node has `nvidia_lock_require_ingest_nonce`, set **`QSDM_NGC_FETCH_CHALLENGE=true`** on the sidecar so it pulls `GET .../ngc-challenge` before building the bundle (HMAC automatically uses **v2** when a nonce is present). The API limits this route to **15/min** per client; on **429** the sidecar waits for **`Retry-After`** and retries (up to **`QSDM_NGC_CHALLENGE_MAX_RETRIES`**, default **4**, max **12**). Optional **`QSDM_NGC_CHALLENGE_JITTER_MAX_SEC`** (or legacy **`QSDM_NGC_CHALLENGE_JITTER_MAX_SEC`**) sleeps a random **0…max** seconds before each challenge request so many validators behind one NAT do not hit the limit in sync.

For dev TLS with self-signed certs on the node, set `QSDM_NGC_REPORT_INSECURE_TLS=true` (or legacy `QSDM_NGC_REPORT_INSECURE_TLS`) on the sidecar (not for production).
...
| `scripts/wire-qsdm.ps1` / `scripts/wire-qsdm.sh` | Export NGC report URL + secret (+ optional proof node id) for `docker compose` |
