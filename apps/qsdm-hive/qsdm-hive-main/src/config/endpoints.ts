/* eslint-disable @cspell/spellchecker */
export enum Endpoints {
  SET_ZOOM = 'SET_ZOOM',
  GET_TASK_SOURCE = 'GET_TASK_SOURCE',
  GET_PRIVATE_TASKS = 'GET_PRIVATE_TASKS',
  GET_TASK_METADATA = 'GET_TASK_METADATA',
  DELEGATE_STAKE = 'DELEGATE_STAKE',
  GET_TASK_INFO = 'GET_TASK_INFO',
  START_TASK = 'START_TASK',
  STOP_TASK = 'STOP_TASK',
  CHECK_WALLET_EXISTS = 'CHECK_WALLET_EXISTS',
  GET_MAIN_ACCOUNT_PUBKEY = 'GET_MAIN_ACCOUNT_PUBKEY',
  GET_STAKING_ACCOUNT_PUBKEY = 'GET_STAKING_ACCOUNT_PUBKEY',
  GET_TASK_LOGS = 'GET_TASK_LOGS',
  GET_MY_TASKS = 'GET_MY_TASKS',
  GET_AVAILABLE_TASKS = 'GET_AVAILABLE_TASKS',
  CLAIM_REWARD = 'CLAIM_REWARD',
  WITHDRAW_STAKE = 'WITHDRAW_STAKE',
  CREATE_NODE_WALLETS = 'CREATE_NODE_WALLETS',
  CREATE_NODE_WALLETS_FROM_JSON = 'CREATE_NODE_WALLETS_FROM_JSON',
  GENERATE_SEED_PHRASE = 'GENERATE_SEED_PHRASE',
  SET_ACTIVE_ACCOUNT = 'SET_ACTIVE_ACCOUNT',
  GET_ALL_ACCOUNTS = 'GET_ALL_ACCOUNTS',
  STORE_USER_CONFIG = 'STORE_USER_CONFIG',
  GET_USER_CONFIG = 'GET_USER_CONFIG',
  GET_TASKS_BY_ID = 'GET_TASKS_BY_ID',
  REMOVE_ACCOUNT_BY_NAME = 'REMOVE_ACCOUNT_BY_NAME',
  OPEN_BROWSER_WINDOW = 'OPEN_BROWSER_WINDOW',
  COPY_TEXT_TO_CLIPBOARD = 'COPY_TEXT_TO_CLIPBOARD',
  GET_TASK_NODE_INFO = 'GET_TASK_NODE_INFO',
  GET_STORED_TASK_VARIABLES = 'GET_STORED_TASK_VARIABLES',
  GET_RPC_STATUS = 'GET_RPC_STATUS',
  GET_QSDM_CORE_STATUS = 'GET_QSDM_CORE_STATUS',
  GET_QSDM_CELL_ACCOUNT = 'GET_QSDM_CELL_ACCOUNT',
  GET_QSDM_MINER_REWARD_STATUS = 'GET_QSDM_MINER_REWARD_STATUS',
  GET_QSDM_MOTHER_HIVE_STATUS = 'GET_QSDM_MOTHER_HIVE_STATUS',
  PAIR_QSDM_MOTHER_HIVE = 'PAIR_QSDM_MOTHER_HIVE',
  DISCONNECT_QSDM_MOTHER_HIVE = 'DISCONNECT_QSDM_MOTHER_HIVE',
  GET_QSDM_VIRTUAL_COMPUTE_RESOURCES = 'GET_QSDM_VIRTUAL_COMPUTE_RESOURCES',
  GET_QSDM_VIRTUAL_COMPUTE_JOBS = 'GET_QSDM_VIRTUAL_COMPUTE_JOBS',
  SUBMIT_QSDM_VIRTUAL_COMPUTE_JOB = 'SUBMIT_QSDM_VIRTUAL_COMPUTE_JOB',
  CANCEL_QSDM_VIRTUAL_COMPUTE_JOB = 'CANCEL_QSDM_VIRTUAL_COMPUTE_JOB',
  GET_QSDM_SKYFANG_LINK_STATUS = 'GET_QSDM_SKYFANG_LINK_STATUS',
  LINK_QSDM_SKYFANG_ACCOUNT = 'LINK_QSDM_SKYFANG_ACCOUNT',
  SET_QSDM_MINER_REWARD_ADDRESS_TO_SIGNER = 'SET_QSDM_MINER_REWARD_ADDRESS_TO_SIGNER',
  CREATE_QSDM_SIGNER_WALLET = 'CREATE_QSDM_SIGNER_WALLET',
  ENABLE_QSDM_SIGNER_LEGACY_RECOVERY = 'ENABLE_QSDM_SIGNER_LEGACY_RECOVERY',
  RESTORE_QSDM_SIGNER_WALLET = 'RESTORE_QSDM_SIGNER_WALLET',
  IMPORT_QSDM_SIGNER_WALLET = 'IMPORT_QSDM_SIGNER_WALLET',
  UNLOCK_QSDM_SIGNER_WALLET = 'UNLOCK_QSDM_SIGNER_WALLET',
  EXPORT_QSDM_SIGNER_WALLET_BACKUP = 'EXPORT_QSDM_SIGNER_WALLET_BACKUP',
  EXPORT_QSDM_SIGNER_RECOVERY_WORDS = 'EXPORT_QSDM_SIGNER_RECOVERY_WORDS',
  GET_QSDM_WALLET_PROVIDER_PERMISSIONS = 'GET_QSDM_WALLET_PROVIDER_PERMISSIONS',
  REVOKE_QSDM_WALLET_PROVIDER_PERMISSION = 'REVOKE_QSDM_WALLET_PROVIDER_PERMISSION',
  CLAIM_QSDM_CELL_FAUCET = 'CLAIM_QSDM_CELL_FAUCET',
  GET_QSDM_REFERRAL_STATUS = 'GET_QSDM_REFERRAL_STATUS',
  GET_QSDM_REFERRAL_REWARD_POOL_STATUS = 'GET_QSDM_REFERRAL_REWARD_POOL_STATUS',
  REGISTER_QSDM_REFERRAL = 'REGISTER_QSDM_REFERRAL',
  CLAIM_QSDM_REFERRAL_REWARD = 'CLAIM_QSDM_REFERRAL_REWARD',
  SUBMIT_QSDM_SIGNED_TRANSACTION = 'SUBMIT_QSDM_SIGNED_TRANSACTION',
  SUBMIT_QSDM_TASK_ACTION = 'SUBMIT_QSDM_TASK_ACTION',
  MANAGE_QSDM_TASK_CATALOG = 'MANAGE_QSDM_TASK_CATALOG',
  RUN_QSDM_SIGNED_CELL_LOOP = 'RUN_QSDM_SIGNED_CELL_LOOP',
  GET_STORED_PAIRED_TASK_VARIABLES = 'GET_STORED_PAIRED_TASK_VARIABLES',
  STORE_TASK_VARIABLE = 'STORE_TASK_VARIABLE',
  GET_TASK_VARIABLES_NAMES = 'GET_TASK_VARIABLES_NAMES',
  FETCH_KPL_TOKEN_METADATA = 'FETCH_KPL_TOKEN_METADATA',
  FETCH_MULTIPLE_KPL_TOKEN_METADATA = 'FETCH_MULTIPLE_KPL_TOKEN_METADATA',
  EDIT_TASK_VARIABLE = 'EDIT_TASK_VARIABLE',
  DELETE_TASK_VARIABLE = 'DELETE_TASK_VARIABLE',
  PAIR_TASK_VARIABLE = 'PAIR_TASK_VARIABLE',
  UNPAIR_TASK_VARIABLE = 'UNPAIR_TASK_VARIABLE',
  GET_TASKS_PAIRED_WITH_VARIABLE = 'GET_TASKS_PAIRED_WITH_VARIABLE',
  GET_TASK_PAIRED_VARIABLES_NAMES_WITH_LABELS = 'GET_TASK_PAIRED_VARIABLES_NAMES_WITH_LABELS',
  GET_ACCOUNT_BALANCE = 'GET_ACCOUNT_BALANCE',
  GET_AVERAGE_SLOT_TIME = 'GET_AVERAGE_SLOT_TIME',
  SWITCH_NETWORK = 'SWITCH_NETWORK',
  GET_NETWORK_URL = 'GET_NETWORK_URL',
  GET_CURRENT_SLOT = 'GET_CURRENT_SLOT',
  INITIALIZE_TASKS = 'INITIALIZE_TASKS',
  OPEN_LOGFILE_FOLDER = 'OPEN_LOGFILE_FOLDER',
  OPEN_NODE_LOGFILE_FOLDER = 'OPEN_NODE_LOGFILE_FOLDER',
  GET_ACTIVE_ACCOUNT_NAME = 'GET_ACTIVE_ACCOUNT_NAME',
  GET_RENT_AMOUNT = 'GET_RENT_AMOUNT',
  GET_VERSION = 'GET_VERSION',
  GET_HIVE_VERSION_POLICY = 'GET_HIVE_VERSION_POLICY',
  GET_ENCRYPTED_SECRET_PHRASE = 'GET_ENCRYPTED_SECRET_PHRASE',
  ARCHIVE_TASK = 'ARCHIVE_TASK',
  DOWNLOAD_APP_UPDATE = 'DOWNLOAD_APP_UPDATE',
  CHECK_APP_UPDATE = 'CHECK_APP_UPDATE',
  GET_PLATFORM = 'GET_PLATFORM',
  STORE_ALL_TIME_REWARDS = 'STORE_ALL_TIME_REWARDS',
  GET_ALL_TIME_REWARDS_BY_TASK = 'GET_ALL_TIME_REWARDS_BY_TASK',
  VALIDATE_PUBLIC_KEY = 'VALIDATE_PUBLIC_KEY',
  GET_RUNNED_PRIVATE_TASKS = 'GET_RUNNED_PRIVATE_TASKS',
  SET_RUNNED_PRIVATE_TASKS = 'SET_RUNNED_PRIVATE_TASKS',
  GET_IS_TASK_RUNNING = 'GET_IS_TASK_RUNNING',
  ENABLE_STAY_AWAKE = 'ENABLE_STAY_AWAKE',
  DISABLE_STAY_AWAKE = 'DISABLE_STAY_AWAKE',
  GET_MAIN_LOGS = 'GET_MAIN_LOGS',
  GET_STARTED_TASKS_PUBKEYS = 'GET_STARTED_TASKS_PUBKEYS',
  UPGRADE_TASK = 'UPGRADE_TASK',
  GET_RUNNING_TASKS_PUBKEYS = 'GET_RUNNING_TASKS_PUBKEYS',
  GET_TIME_TO_NEXT_REWARD = 'GET_TIME_TO_NEXT_REWARD',
  CANCEL_TASK_RETRY = 'CANCEL_TASK_RETRY',
  GET_RETRY_DATA_BY_TASK_ID = 'GET_RETRY_DATA_BY_TASK_ID',
  SWITCH_LAUNCH_ON_RESTART = 'SWITCH_LAUNCH_ON_RESTART',
  STOP_ALL_TASKS = 'STOP_ALL_TASKS',
  START_ALL_TASKS = 'START_ALL_TASKS',
  ADD_TASKS_SCHEDULE = 'ADD_TASKS_SCHEDULE',
  REMOVE_TASKS_SCHEDULE = 'REMOVE_TASKS_SCHEDULE',
  UPDATE_TASKS_SCHEDULE_BY_ID = 'UPDATE_TASKS_SCHEDULE_BY_ID',
  GET_TASKS_SCHEDULES = 'GET_TASKS_SCHEDULES',
  GET_TASKS_SCHEDULE_BY_ID = 'GET_TASKS_SCHEDULE_BY_ID',
  ADD_TASK_TO_SCHEDULER = 'ADD_TASK_TO_SCHEDULER',
  GET_SCHEDULER_TASKS = 'GET_SCHEDULER_TASKS',
  REMOVE_TASK_FROM_SCHEDULER = 'REMOVE_TASK_FROM_SCHEDULER',
  VALIDATE_SCHEDULER_SESSION = 'VALIDATE_SCHEDULER_SESSION',
  CREDIT_STAKING_WALLET_FROM_MAIN_WALLET = 'CREDIT_STAKING_WALLET_FROM_MAIN_WALLET',
  LIMIT_LOGS_SIZE = 'LIMIT_LOGS_SIZE',
  START_NETWORK_MIGRATION = 'START_NETWORK_MIGRATION',
  FINISH_NETWORK_MIGRATION = 'FINISH_NETWORK_MIGRATION',
  GET_TASK_LAST_SUBMISSION_TIME = 'GET_TASK_LAST_SUBMISSION_TIME',
  REDEEM_TOKENS_IN_NEW_NETWORK = 'REDEEM_TOKENS_IN_NEW_NETWORK',
  TRANSFER_CELL_FROM_MAIN_WALLET = 'TRANSFER_CELL_FROM_MAIN_WALLET',
  TRANSFER_CELL_FROM_STAKING_WALLET = 'TRANSFER_CELL_FROM_STAKING_WALLET',
  GET_NOTIFICATIONS = 'GET_NOTIFICATIONS',
  STORE_NOTIFICATION = 'STORE_NOTIFICATION',
  PURGE_NOTIFICATIONS = 'PURGE_NOTIFICATIONS',
  REMOVE_NOTIFICATION = 'REMOVE_NOTIFICATION',
  UPDATE_NOTIFICATION = 'UPDATE_NOTIFICATION',
  FETCH_S3_FOLDER_CONTENTS = 'FETCH_S3_FOLDER_CONTENTS',
  GET_ENCRYPTED_SECRET_PHRASE_MAP = 'GET_ENCRYPTED_SECRET_PHRASE_MAP',
  SAVE_ENCRYPTED_SECRET_PHRASE_MAP = 'SAVE_ENCRYPTED_SECRET_PHRASE_MAP',
  APP_RELAUNCH = 'APP_RELAUNCH',
  QUIT_APP = 'QUIT_APP',
  FETCH_AND_SAVE_UPNP_BIN = 'FETCH_AND_SAVE_UPNP_BIN',
  CHECK_UPNP_BINARY = 'CHECK_UPNP_BINARY',
  VALIDATE_BIP39_WORD = 'VALIDATE_BIP39_WORD',
  CHECK_IF_STAKING_ACCOUNT_IS_FAULTY = 'CHECK_IF_STAKING_ACCOUNT_IS_FAULTY',
  GET_TASK_ID_BY_TASK_AUDIT_PROGRAM_ID = 'GET_TASK_ID_BY_TASK_AUDIT_PROGRAM_ID',
  SAVE_TASK_THUMBNAIL = 'SAVE_TASK_THUMBNAIL',
  GET_TASK_THUMBNAIL = 'GET_TASK_THUMBNAIL',
  GET_MY_TASK_STAKE = 'GET_MY_TASK_STAKE',
  GET_MY_TASK_SUBMISSION_INFO = 'GET_MY_TASK_SUBMISSION_INFO',
  RECOVER_STAKING_ACCOUNT = 'RECOVER_STAKING_ACCOUNT',
  GET_TASK_SUBMISSIONS = 'GET_TASK_SUBMISSIONS',
  FETCH_KPL_LIST = 'FETCH_KPL_LIST',
  GET_KPL_BALANCE = 'GET_KPL_BALANCE',
  TRANSFER_KPL = 'TRANSFER_KPL',
  RESET_PIN = 'RESET_PIN',
  RECOVER_LOST_TOKENS = 'RECOVER_LOST_TOKENS',
  RESET_NODE_CACHE = 'RESET_NODE_CACHE',
  GET_KPL_STAKING_ACCOUNT_PUBKEY = 'GET_KPL_STAKING_ACCOUNT_PUBKEY',
  CLAIM_REWARD_KPL = 'CLAIM_REWARD_KPL',
  CREATE_MISSING_KPL_STAKING_KEY = 'CREATE_MISSING_KPL_STAKING_KEY',
  RECOVER_KPL_STAKING_ACCOUNT = 'RECOVER_KPL_STAKING_ACCOUNT',
  SET_IS_ACCOUNT_HIDDEN = 'SET_IS_ACCOUNT_HIDDEN',
  GET_IS_ACCOUNT_HIDDEN = 'GET_IS_ACCOUNT_HIDDEN',
  VERIFY_MESSAGE = 'VERIFY_MESSAGE',
  CREATE_REFERRAL_CODE = 'CREATE_REFERRAL_CODE',
  CHECK_VIP_ACCESS = 'CHECK_VIP_ACCESS',
  TOGGLE_THEME = 'TOGGLE_THEME',
  GET_OPERATIVE_SYSTEM = 'GET_OPERATIVE_SYSTEM',
  VALIDATE_BRANDING_FOLDER = 'VALIDATE_BRANDING_FOLDER',
  COPY_BRANDING_FOLDER = 'COPY_BRANDING_FOLDER',
  GET_BRANDING_CONFIG = 'GET_BRANDING_CONFIG',
  GET_BRAND_LOGO = 'GET_BRAND_LOGO',
}

export enum RendererEndpoints {
  UPDATE_AVAILABLE = 'UPDATE_AVAILABLE',
  SYSTEM_WAKE_UP = 'SYSTEM_WAKE_UP',
  UPDATE_DOWNLOADED = 'UPDATE_DOWNLOADED',
  TASK_EXECUTABLE_CHANGED = 'TASK_EXECUTABLE_CHANGED',
  TASK_NOTIFICATION_RECEIVED = 'TASK_NOTIFICATION_RECEIVED',
  UPDATE_MY_NODE_TASKS = 'UPDATE_MY_NODE_TASKS',
  K2_RATE_LIMIT = 'K2_RATE_LIMIT',
  K2_CONNECTION_ERROR = 'K2_CONNECTION_ERROR',
  NAVIDATE_TO_ROUTE = 'NAVIDATE_TO_ROUTE',
  TASK_VARIABLES_UPDATED = 'TASK_VARIABLES_UPDATED',
}
