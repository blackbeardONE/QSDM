...
async function handleServerStart(server: any, expressApp: any) {
  try {
    ...
    if (forceTunneling) {
      log('Force tunneling enabled; skipping UPnP');
    ...
    const isOptionalUpnpFailure =
      message.includes('QSDM UPnP binary download is not configured') ||
      message.includes('QSDM UPnP binary downloads must use HTTPS') ||
      message.includes('QSDM UPnP binary URL redirects too many times');

    logToConsoleAndToLogFile(
      isOptionalUpnpFailure
        ? `UPnP unavailable; falling back to Network Tunneling: ${message}`
        : `An error occurred while trying UPnP: ${message}`,
      !isOptionalUpnpFailure
    );
    await setUpLocalTunnel();
  }
