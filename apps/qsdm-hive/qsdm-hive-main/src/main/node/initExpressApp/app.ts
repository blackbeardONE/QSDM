...
import {
  createTaskVariable,
  upsertTaskVariable,
} from 'main/controllers/taskVariables/createTaskVariableLink';
...
const verifyTaskEndpoints = (
  ...
  if (match) {
    ...
    if (taskId?.length === 43) {
      isTask = true;
      taskActivated = !qsdmHiveState.getAddedTasks().every((addedTask) => {
...
export default (): Express => {
  ...
  app.use(
    ...
    proxy(
      function (req: any) {
        ...
        if (qsdmHiveTasks.RUNNING_TASKS[taskId]) {
          console.log(
            `http://localhost:${qsdmHiveTasks.RUNNING_TASKS[taskId].expressAppPort}`
          );
          return `http://localhost:${qsdmHiveTasks.RUNNING_TASKS[taskId].expressAppPort}`;
        }
  ...
  app.post('/api/task-variables-upsert', upsertTaskVariable);
