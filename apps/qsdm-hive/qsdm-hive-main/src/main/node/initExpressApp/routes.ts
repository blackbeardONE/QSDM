...
import { tryHandleQsdmNativeNamespaceCall } from 'main/services/qsdmNativeNamespace';
...
export default (app: Express) => {
  ...
  app.post('/namespace-wrapper', async (req, res) => {
    ...
    const runningTask = qsdmHiveTasks.RUNNING_TASKS[taskId];
    if (!runningTask) {
      return res.status(404).send({ message: 'Task is not running' });
    }
    if (runningTask.secret !== req.body.secret) {
      return res.status(401).send({ message: 'Invalid secret provided' });
    }
    try {
      const params = args.slice(1);
      const method = args[0];
      const nativeResponse = await tryHandleQsdmNativeNamespaceCall({
        taskId,
        method,
        params,
        taskData: (runningTask as any).namespace?.taskData,
      });
      if (nativeResponse.handled) {
        return res.status(200).send({ response: nativeResponse.response });
      }

      const response = await (runningTask as any).namespace[method](...params);
