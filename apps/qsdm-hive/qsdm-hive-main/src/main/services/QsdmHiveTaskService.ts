...
import {
  ...
  requireQsdmSkyFangWalletLinkedForMiner,
...
const qsdmNativeTaskUrls = (path: string) =>
...
const fetchQsdmNativeJson = async <T,>(
  ...
  for (const url of qsdmNativeTaskUrls(path)) {
    try {
...
export class QsdmHiveTaskService {
  ...
  async initializeTasks() {
    if (QSDM_TASK_RUNTIME_MODE === 'qsdm-native') {
      ...
      try {
        ...
        if (runningTasks.includes(QSDM_EDGE_WORKER_SYSTEM_TASK_ID)) {
  ...
  private async fetchQsdmNativeTasks(): Promise<RawTaskData[]> {
    try {
      ...
      console.warn('QSDM native task list fetch failed; falling back to cache.', error);
  ...
  public async reconcileQsdmSystemTaskRuntime(
    ...
    if (
      ...
      (!isMinerSystemTask && !isEdgeWorkerSystemTask && !isSkyFangLinkSystemTask)
    ) {
      if (isSkyFangLinkSystemTask) {
        console.log(
          'QSDM Sky Fang Link is a one-time verifier; it is not auto-restored.'
        );
      }
    ...
    if (this.RUNNING_TASKS[taskAccountPubKey]) {
      if (isMinerSystemTask) {
        try {
          ...
          console.warn(
            'QSDM miner is running, but Sky Fang wallet-link verification failed; stopping the miner task.',
            error
          );
          try {
            await this.stopTask(taskAccountPubKey);
          } catch (stopError) {
            console.warn(
              'Could not stop the unlinked QSDM miner task cleanly.',
              stopError
            );
          }
          return false;
      ...
      if (isSkyFangLinkSystemTask) {
        try {
          ...
          console.warn(
            'QSDM Sky Fang Link is running, but Sky Fang live wallet-link verification failed; stopping the verifier task.',
            error
          );
          try {
            await this.stopTask(taskAccountPubKey);
            ...
            console.warn(
              'Could not stop the unlinked QSDM Sky Fang Link task cleanly.',
              stopError
            );
          }
          return false;
    ...
    try {
      ...
      console.warn(
        'QSDM miner process is running, but task stake ownership could not be verified.',
        error
      );
      return false;
    }

    if (ownership.currentStakeCell <= 0) {
      console.warn(
    ...
    if (isMinerSystemTask) {
      try {
        ...
        console.warn(
          'QSDM miner process was not restored because Sky Fang wallet-link verification failed.',
          error
        );
        return false;
      }
    ...
    if (isEdgeWorkerSystemTask) {
      ...
      const { child, secret } = startQsdmEdgeWorkerSystemProcess(taskInfo);
    ...
    if (options.submitStartAction && !ownership.runningForCurrentSender) {
      try {
        ...
        console.warn(
          'QSDM miner process is running, but the signed start action was not accepted; leaving it unmanaged.',
          error
        );
        return false;
      }
  ...
  async fetchAllTaskIds() {
    if (QSDM_TASK_RUNTIME_MODE === 'qsdm-native') {
      ...
      this.allTaskPubkeys = tasks.map((task) => task.task_id);
      this.kplTaskPubKeys = [];
      this.privateTaskPubKeys = [];
      return;
    }
