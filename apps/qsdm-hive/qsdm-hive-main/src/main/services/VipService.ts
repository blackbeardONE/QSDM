...
const LEGACY_THEME_KEY = ['ko', 'ii'].join('');
...
type LegacyVipState = Record<
  string,
  {
    hasVipAccess: boolean;
    theme: 'vip' | 'qsdm' | string;
    lastFetchTimestamp: number;
  }
>;
...
class VipService {
  ...
  private static readonly VIP_TOKEN_THRESHOLD = getBaseUnitsFromCell(1000); // Minimum amount of tokens needed for VIP status
    ...
    if (
      ...
      return currentState[walletAddress].hasVipAccess &&
        ...
        : currentState[walletAddress].hasVipAccess &&
          currentState[walletAddress].theme === 'qsdm'
        ? 'vip-disabled'
        : 'no-access';
    }
  ...
  private getVipState(): VipState {
    ...
    try {
      ...
      const legacyState = JSON.parse(decrypted) as LegacyVipState;
      const state = Object.fromEntries(
        Object.entries(legacyState).map(([walletAddress, entry]) => [
          walletAddress,
          {
            ...entry,
            theme:
              entry.theme === 'vip' && entry.theme !== LEGACY_THEME_KEY
                ? 'vip'
                : 'qsdm',
          },
        ])
