...
const getBaseTaskActionSignerPaths = () => {
  const discovered = mirrorDiscoveredSignerToAppData(
    getDiscoveredLocalSigner()
  );
    ...
    for (const walletPath of listFiles(signerDir, 'wallet.json')) {
      ...
      if (sameAddress(walletAddress, address)) {
        return {
          ...
          passphraseFile: findPassphraseForWallet(walletPath, passphraseFile),
        };
      }
    }
  }
...
const getDiscoveredLocalSigner = () => {
  ...
  const qsdmCliName = process.platform === 'win32' ? 'qsdmcli.exe' : 'qsdmcli';
  const platformDir = process.platform === 'win32' ? 'win' : process.platform;
  const { resourcesPath } = process as NodeJS.Process & {
    resourcesPath?: string;
  };
  const cliPath = findFirstExistingFile([
    resourcesPath ? path.join(resourcesPath, 'native', qsdmCliName) : '',
    path.join(process.cwd(), 'native', platformDir, 'x64', qsdmCliName),
    ...roots.flatMap((root) => [
      path.join(
        root,
        'apps',
...
const resolveTaskActionSigner = () => {
  ...
  const keystoreAddress = readWalletAddress(base.cliPath, base.keystorePath);
...
export const getQsdmTaskActionCliPath = () => resolveTaskActionSigner().cliPath;
...
export const getQsdmLocalSignedLoopEnabled = () =>
  QSDM_ENABLE_LOCAL_SIGNED_LOOP ||
  (!!getQsdmTaskActionKeystorePath() && trustedCoreApiEnabled());
