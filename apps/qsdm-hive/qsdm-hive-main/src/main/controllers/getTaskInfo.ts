...
import { K2TasksDataFetchOptions } from 'main/services/QsdmHiveTaskService/types';
import {
  ErrorType,
  GetTaskInfoParam,
  GetTaskInfoResponse,
...
export const getTaskInfo = async (
  ...
  try {
    ...
    if (!forceTaskRefetch) {
      partialRawTaskData = (await qsdmHiveTasks.getStartedTasks()).find(
    ...
    if (!partialRawTaskData || forceTaskRefetch) {
      partialRawTaskData = (await qsdmHiveTasks.getTaskState(
        taskAccountPubKey,
        options
      )) as RawTaskData;
    }
