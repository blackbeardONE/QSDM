...
export const getPrivateTasks = async (
  ...
  try {
    ...
    const privateTaskIds: string[] = qsdmHiveTasks.privateTaskPubKeys;
    const runningIds = await qsdmHiveTasks.getStartedTasksPubKeys();
    ...
    const rawTasks =
      QSDM_TASK_RUNTIME_MODE === 'qsdm-native'
        ? await qsdmHiveTasks.fetchTasksData(idsSlice)
        : await fetchTasksWithTimeout(idsSlice);
