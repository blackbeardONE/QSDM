...
export const delegateStakeKPL = async (
  ...
  const initializeCellStakingAccount = async () => {
    ...
    if (!isStakingAccountInitialized) {
      ...
      if (stakingAccountIsCorrectlyOwnedByTaskProgram) {
        ...
        const cellTransaction = new Transaction().add(
        ...
        try {
          ...
          console.log(
            `${NATIVE_TOKEN_SYMBOL} staking account initialized successfully`
          );
        } catch (error) {
          console.log(
            `${NATIVE_TOKEN_SYMBOL} staking account not initialized:`,
            error
          );
        }
      }
  ...
  const [cellResult, mainStakingResult] = await Promise.allSettled([
    initializeCellStakingAccount(),
