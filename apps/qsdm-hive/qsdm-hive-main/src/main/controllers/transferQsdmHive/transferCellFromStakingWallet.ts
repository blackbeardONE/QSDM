...
import { TransferCellParam } from 'models';
import { throwTransactionError } from 'utils/error';

import { getWalletKeyPairByType } from '../../node/helpers';

import { transferCellFromMainWallet } from './transferCellFromMainWallet';
...
export const transferCellFromStakingWallet = async (
  ...
  if (QSDM_TASK_RUNTIME_MODE === 'qsdm-native') {
    throw new Error(
      'Native CELL transfers must use the QSDM signer wallet, not a legacy staking key'
    );
  }
  ...
  try {
    ...
    if (stakingKeyIsCorrectlyOwned) {
      ...
      await transferCellFromMainWallet({} as Event, {
        accountName,
        amount,
        toWalletAddress,
      });
