...
import { calculateTxFee, CELL_BASE_UNITS_PER_CELL } from 'utils/calculateTxFee';
...
export async function transferToken({
  ...
  const fromTokenAccount = await getOrCreateAssociatedTokenAccount(
    connection as unknown as QsdmConnection,
    fromKeypair,
  ...
  const mintInfo = await getMint(
    sdk.k2Connection as unknown as QsdmConnection,
  ...
  if (balance < estimatedFees) {
    return throwDetailedError({
      detailed: `Insufficient balance to cover transaction fees. Required: ${
        estimatedFees / CELL_BASE_UNITS_PER_CELL
      } ${NATIVE_TOKEN_SYMBOL}, Available: ${
        balance / CELL_BASE_UNITS_PER_CELL
      } ${NATIVE_TOKEN_SYMBOL}`,
