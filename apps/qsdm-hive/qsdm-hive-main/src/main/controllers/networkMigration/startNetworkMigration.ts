...
export const startNetworkMigration = async (
  ...
  const allMyTasks = await qsdmHiveTasks.getStartedTasksPubKeys();
...
const archiveOldTasks = async () => {
  const tasksFromOldNetwork = await qsdmHiveTasks.getStartedTasksPubKeys();

  tasksFromOldNetwork.forEach((taskPublicKey) =>
    qsdmHiveTasks.removeTaskFromStartedTasks(taskPublicKey)
  );
};
...
const recordTasksAndStakesForAccount = async (
  ...
  for (const taskPublicKey of Object.keys(taskStateMap)) {
    try {
      const taskType = taskStateMap[taskPublicKey]?.tokenType ? 'KPL' : 'CELL';
