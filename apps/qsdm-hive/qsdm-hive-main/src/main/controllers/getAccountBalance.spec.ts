...
describe('getAccountBalance', () => {
  ...
  it('retains an expired confirmed balance during a transient outage', async () => {
    const pubkey = 'GBFfGukraHc3B6iBF5LWKqPkBRkzAzmsziKzVvRhA1oR';
    const now = jest.spyOn(Date, 'now').mockReturnValue(1000);
    (sdk.k2Connection.getBalance as jest.Mock).mockResolvedValueOnce(42);

    expect(await getAccountBalance({} as Event, pubkey)).toBe(42);
