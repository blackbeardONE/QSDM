...
export const delegateStakeCell = async (
  _: Event,
  ...
  if (mainStakingResult.status === 'fulfilled') {
    await saveStakeRecordToCache(
      ...
      getBaseUnitsFromCell(stakeAmount),
      true
    );
    ...
    console.log(
      `Successfully staked ${stakeAmount} ${NATIVE_TOKEN_SYMBOL} into the task ${taskAccountPubKey} from ${NATIVE_TOKEN_SYMBOL} staking account ${stakingAccKeypair.publicKey.toBase58()}`
    );
