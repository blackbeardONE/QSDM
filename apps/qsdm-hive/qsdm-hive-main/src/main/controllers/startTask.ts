...
const buildMissingQsdmStakeMessage = async (
  taskId: string,
  sender: string
) => {
  try {
    ...
    if (ownership.foreignParticipants.length > 0) {
      ...
      return `Can't start task ${taskId}: active QSDM signer ${currentSigner} has 0 CELL staked on this task. Existing stake is bound to another signer: ${foreignOwners}. QSDM task stake is wallet-bound; import the signer wallet that owns that stake, or add a one-time stake with the current signer.`;
    }

    return `Can't start task ${taskId}: active QSDM signer ${currentSigner} has no CELL stake on this task. Add the task's one-time stake with the current signer before starting.`;
  } catch (error: any) {
...
const startTask = async (
  ...
  if (
    ...
    (isSystemMinerTask || isSystemSkyFangLinkTask)
  ) {
    try {
      if (isSystemSkyFangLinkTask) {
  ...
  if (isQsdmNativeTaskRuntime && !(isSystemTask && isTaskRunning)) {
    try {
