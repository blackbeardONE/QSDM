...
import { validateIpcPayload } from './security/ipcValidation';
import { assertHiveVersionPolicyAllowsEndpoint } from './services/hiveVersionPolicy';
...
> = {
  ...
  [Endpoints.COPY_TEXT_TO_CLIPBOARD]: controllers.copyTextToClipboard,
  ...
  [Endpoints.GET_QSDM_CORE_STATUS]: controllers.getQsdmCoreStatus,
  ...
  [Endpoints.EXPORT_QSDM_SIGNER_WALLET_BACKUP]:
    controllers.exportQsdmSignerWalletBackup,
  [Endpoints.EXPORT_QSDM_SIGNER_RECOVERY_WORDS]:
    controllers.exportQsdmSignerRecoveryWords,
  [Endpoints.GET_QSDM_WALLET_PROVIDER_PERMISSIONS]:
    controllers.getQsdmWalletProviderPermissionsController,
  [Endpoints.REVOKE_QSDM_WALLET_PROVIDER_PERMISSION]:
    controllers.revokeQsdmWalletProviderPermissionController,
  [Endpoints.CLAIM_QSDM_CELL_FAUCET]: controllers.claimQsdmCellFaucet,
  [Endpoints.GET_QSDM_REFERRAL_STATUS]:
    controllers.getQsdmReferralStatusController,
  [Endpoints.GET_QSDM_REFERRAL_REWARD_POOL_STATUS]:
    controllers.getQsdmReferralRewardPoolStatusController,
  [Endpoints.REGISTER_QSDM_REFERRAL]:
    controllers.registerQsdmReferralController,
  [Endpoints.CLAIM_QSDM_REFERRAL_REWARD]:
    controllers.claimQsdmReferralRewardController,
  [Endpoints.SUBMIT_QSDM_SIGNED_TRANSACTION]:
    controllers.submitQsdmSignedTransaction,
  [Endpoints.SUBMIT_QSDM_TASK_ACTION]: controllers.submitQsdmTaskAction,
  [Endpoints.MANAGE_QSDM_TASK_CATALOG]: controllers.manageQsdmTaskCatalog,
  [Endpoints.RUN_QSDM_SIGNED_CELL_LOOP]: controllers.runQsdmSignedCellLoop,
  ...
  [Endpoints.GET_HIVE_VERSION_POLICY]: controllers.getHiveVersionPolicy,
  ...
  [Endpoints.TRANSFER_CELL_FROM_MAIN_WALLET]:
    controllers.transferCellFromMainWallet,
  [Endpoints.TRANSFER_CELL_FROM_STAKING_WALLET]:
    controllers.transferCellFromStakingWallet,
  ...
  [Endpoints.QUIT_APP]: controllers.quitApp,
...
const initHandlers = (): void => {
  Object.entries(endpointToControllerMap).forEach(
    ([endpointKey, controller]) => {
      ipcMain.handle(
        endpointKey,
        mainErrorHandler(async (event: Event, ...args: unknown[]) => {
          validateIpcPayload(endpointKey, args);
          await assertHiveVersionPolicyAllowsEndpoint(endpointKey);
