...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const [theme, setLocalTheme] = useState<ThemeType>(() => {
    ...
    if (savedTheme === LEGACY_THEME_KEY) {
      localStorage.setItem('app-theme', 'qsdm');
      return 'qsdm';
    }
    return (savedTheme as ThemeType) || 'qsdm';
  });
    ...
    () =>
      window.main.checkVipAccess().then((accessLevel) => {
        ...
        if (accessLevel === 'vip-enabled' && theme !== 'vip') {
          ...
          // If user lost VIP access, switch back to qsdm
          setLocalTheme('qsdm');
          localStorage.setItem('app-theme', 'qsdm');
        }
        return accessLevel;
  ...
  useEffect(() => {
    ...
    if (effectiveBrandingConfig && theme === 'qsdm') {
      colors = {
        ...colors,
