...
export const useSidebraLogic = () => {
  ...
  const { data: nodeInfoData, isLoading: isLoadingNodeInfoData } = useQuery(
    ...
    getTaskNodeInfo,
    {
      ...
      onSettled: (nodeInfo) => {
        const hasRewards = [
          ...Object.values(nodeInfo?.pendingRewards || {}),
          ...Object.values(nodeInfo?.allTimeRewards || {}),
        ].some((reward) => reward > 0);
  ...
  const { newRewardsAvailable } = usePreviousRewards(
    nodeInfoData?.allTimeRewards || nodeInfoData?.pendingRewards
  );
  ...
  const handleSuccess = async () => {
    ...
    >([QueryKeys.taskNodeInfo], (oldNodeData: GetTaskNodeInfoResponse) => ({
      ...
      allTimeRewards: oldNodeData?.allTimeRewards || {},
    }));
