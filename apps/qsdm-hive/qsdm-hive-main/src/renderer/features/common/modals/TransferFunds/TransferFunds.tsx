...
export const TransferFunds = create<PropsType>(function AddStake({
  ...
  const sendTokensApiCall = async () => {
    ...
    if (accountType === 'SYSTEM') {
      return transferCellFromMainWallet(accountName, amount, trimmedWallet);
    }
    return transferCellFromStakingWallet(accountName, amount, trimmedWallet);
  };
  ...
  const {
    ...
    onError: (sendError) => {
      ...
      toast.error(`CELL transfer failed: ${message}`);
    },
  });

  const getWalletAccountBalance = async () => {
    const balanceInBaseUnits = await getAccountBalance(walletAddress);
    const balanceInCell = getCellFromBaseUnits(balanceInBaseUnits);
    return balanceInCell;
  };
      ...
      <ModalContent className="w-[700px]" theme={Theme.Dark}>
        ...
        <div className="flex flex-col items-center justify-center gap-5 py-8 text-white h-84">
          ...
          <div>
            ...
            <QsdmHiveInput onInputChange={handleInputChange} className="w-80" />
          ...
          <div className="py-2 text-xs text-finnieTeal-100">
            {!error
              ? `${balance} ${NATIVE_TOKEN_SYMBOL} available in your balance`
              : '                   '}
          </div>
          {isSending ? (
