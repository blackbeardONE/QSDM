  ...
  const handleCreateKPLStakingKey = async () => {
    ...
    if (isPinValid && settings?.pin) {
      ...
      if (!encryptedSecretPhrase) {
        return setError(
          "It looks like this local Hive profile does not have a recovery phrase"
        );
      }
      try {
        ...
        try {
          ...
          console.log('Second attempt of decrypting the Hive recovery phrase failed');
  ...
  return (
    <div className="text-white">
      <p className="px-12 mt-8 tracking-tight w-full items-start text-start leading-[32px] text-base">
        A legacy task key is needed only for old KPL-compatible tasks. QSDM
        CELL tasks use the QSDM Core signer keystore instead. We will create
        this legacy key for your current account{' '}
