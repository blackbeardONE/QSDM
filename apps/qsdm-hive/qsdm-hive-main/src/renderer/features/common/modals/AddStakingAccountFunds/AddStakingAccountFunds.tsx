import { CloseLine, CurrencyMoneyLine, Icon } from 'vendor/qsdm-styleguide';
...
const getDefaultFundValueInCELL = (accountBalance: number) => {
...
export const AddStakingAccountFunds = create<PropsType>(
  function AddStakingAccountFunds() {
    ...
    const accountBalanceInCELL =
      accountBalance && getCellFromBaseUnits(accountBalance);
    ...
    useEffect(() => {
      const defaultFundValue =
        accountBalanceInCELL && getDefaultFundValueInCELL(accountBalanceInCELL);
      setInputValue(defaultFundValue);
    }, [accountBalanceInCELL]);

    const handleStakingWalletFund = async () => {
      if (!inputValue) {
        return;
      }
      const valueInBaseUnits = inputValue && getBaseUnitsFromCell(inputValue);
      await fundStakingWalletFromMainWallet(valueInBaseUnits);
    };
                ...
                <>
                  ...
                  <div className="h-6 text-center">
                    {displayNotEnoughFundsError ? (
                      ...
                      <div className="text-sm text-green-2">
                        {`${accountBalanceInCELL} ${NATIVE_TOKEN_SYMBOL} available in your balance`}
                      </div>
                    )}
                  </div>
                </>
