  ...
  const handlePinCreate = async () => {
    try {
      if (
        ...
        /**
         * Since users with the previous version of the app might have their
         * Hive recovery phrases encrypted with the old pin, try the old pin as
         * well.
         */
          ...
          async (encryptedSecretPhrase, key) => {
            try {
              ...
              try {
                ...
                /**
                 * Attempt to decode the Hive recovery phrase with the old pin
                 * hash from user settings.
                 */
                const newEncryptedValue = await encryptSeedPhraseWithNewPin(
                  userConfig?.pin,
                  pinConfirm,
                ...
                console.error(
                  `Decryption of the Hive recovery phrase for the Public Key ${key} failed for both raw and hashed pins:`
                );
                throw new Error(
                  `Decryption of the Hive recovery phrase for the Public Key ${key} failed`
                );
              }
            }
          }
        ...
        try {
          ...
          // Save new pin and encrypted Hive recovery phrases.
          ...
          {oldPinError ? (
            ...
            <div className="pt-4 text-xs text-finnieOrange">
              {!pinIsMatching && pinsLengthIsMatching ? (
                ...
                <span>
                  If you forgot your PIN you will need to restore your Hive
                  profile.
                </span>
              )}
            </div>
