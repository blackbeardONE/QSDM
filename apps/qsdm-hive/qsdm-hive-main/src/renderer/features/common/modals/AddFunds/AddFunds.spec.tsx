...
Object.defineProperty(window, 'main', {
  value: {
    ...
    copyTextToClipboard: jest.fn(() => Promise.resolve({ copied: true })),
...
describe('AddFunds', () => {
  ...
  it('copies the public key to the clipboard when clicking on the `copy` button', async () => {
    ...
    expect(window.main.copyTextToClipboard).toHaveBeenCalledWith({
      text: publicKey,
    });
  });

  it('does not offer the local faucet while connected through the gateway', async () => {
    (window.main.getQsdmCoreStatus as jest.Mock).mockResolvedValueOnce({
      healthy: true,
      coreConnectionMode: 'gateway',
      checkedAt: new Date().toISOString(),
    });
    renderSidebar();

    await userEvent.click(screen.getByTestId('sidebar_tip_give_button'));

    expect(await screen.findByText('Receive CELL')).toBeInTheDocument();
    expect(screen.queryByText(/^Claim CELL$/i)).not.toBeInTheDocument();
  });
});
