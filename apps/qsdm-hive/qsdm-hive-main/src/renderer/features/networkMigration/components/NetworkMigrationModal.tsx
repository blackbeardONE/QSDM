...
import { BrowseInternetLine, Button } from 'vendor/qsdm-styleguide';
  ...
  function NetworkMigration({ migrationPhase, newNetworkRpcUrl }) {
    ...
    const skipMigration = useCallback(async () => {
      ...
      if (migrationPhase === 2 && !hasBalance) {
        const confirmed = window.confirm(
          `If you proceed without having ${NATIVE_TOKEN_SYMBOL} tokens, we won't be able to start your tasks on mainnet. Would you like to continue?`
        );
        if (!confirmed) return;
      }
        ...
        {isInitialLoading ? (
          ...
          <>
            <p>
              ...
              you&apos;ll need to have {NATIVE_TOKEN_SYMBOL} tokens in your
              wallet.
            </p>
            <p>
            ...
            <div className="bg-finnieBlue-darker/30 p-4 rounded-lg mx-auto">
              ...
              <p className="text-xl font-medium">
                {accountBalance?.toFixed(2) || '0'} {NATIVE_TOKEN_SYMBOL}
              </p>
            </div>
