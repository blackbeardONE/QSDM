...
import { CloseIcon } from '../SchedulerIcons';
...
export function Session({
  ...
  useEffect(() => {
    setSchedule(scheduleMetadata);
    setShowStopTimer(!!scheduleMetadata.stopTime);
  }, [scheduleMetadata]);
  ...
  const handleAddStopTimer = async () => {
    const startTime = schedule.startTime || scheduleMetadata.startTime;
      ...
      <div className="flex items-center">
        ...
        >
          {displaySessionRemoveButton && (
            <CloseIcon className="w-5 h-5 text-white cursor-pointer" />
          )}
        </div>
      </div>
    </Tooltip>
