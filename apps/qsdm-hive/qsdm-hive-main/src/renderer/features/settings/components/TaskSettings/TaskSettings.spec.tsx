...
jest.mock('@formkit/auto-animate/react', () => ({
  useAutoAnimate: jest.fn(() => ({ current: null })),
}));
...
const inspectModalDescription =
  'QSDM Hive stores this extension locally and passes it only to tasks that explicitly request the paired variable. The value is masked in this view.';
...
describe('TaskSettings', () => {
    ...
    it('displays the right information for each task variable', async () => {
      ...
      for (const existingTaskVariableData of existingTaskVariablesData) {
        ...
        const key = within(inspectModal).getByText(
          `Hidden (${existingTaskVariableData.value.length} characters)`
        );

        expect(label).toBeInTheDocument();
        expect(key).toBeInTheDocument();
        expect(
          within(inspectModal).queryByText(existingTaskVariableData.value)
  ...
  describe('Add task variable', () => {
    it('displays the proper error and disables the Add button if the label entered is not valid', async () => {
      ...
      const variableLabelInput = screen.getByPlaceholderText(/Example:/i);
    ...
    it('disables the Add button if there is no label or key value entered, and enables it when both are there (and the label is valid)', async () => {
      ...
      const variableKeyInput = screen.getByPlaceholderText(
        /Paste token, API key, or task value/i
      );
...
describe('Edit task variable', () => {
  ...
  it('edits correctly a task variable', async () => {
    ...
    const variableKeyInput = within(editModal).getByPlaceholderText(
      /Paste token, API key, or task value/i
    );
    ...
    const key = within(inspectModal).getByText(
      `Hidden (${newTaskVariableData.value.length} characters)`
    );

    expect(label).toBeInTheDocument();
    expect(key).toBeInTheDocument();
    expect(
      within(inspectModal).queryByText(newTaskVariableData.value)
    ).not.toBeInTheDocument();
  });
});
