...
export function QsdmWalletPanel() {
  ...
  const {
    ...
    () =>
      transferCellFromMainWallet('__qsdm_signer__', amountNumber, recipient),
    {
      onSuccess: async () => {
        setRecipient('');
        setAmount('');
  ...
  return (
    <section className="w-[90%] p-5 mb-6 rounded-lg border border-purple-1 bg-purple-1 bg-opacity-20">
      ...
      <div className="grid grid-cols-1 gap-3 pt-5 md:grid-cols-[1fr_220px_150px]">
        <div>
          <label
      ...
      <div className="pt-2 text-xs text-finnieGray-secondary">
        Core: {QSDM_BRIDGE_CONFIG.coreApiUrl}
      </div>
    </section>
  );
}
