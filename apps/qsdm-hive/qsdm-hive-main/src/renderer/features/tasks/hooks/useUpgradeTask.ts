...
export const useUpgradeTask = ({
  ...
  const newTaskVersionTotalBountyInCell = getCellFromBaseUnits(
    newTaskVersion?.totalBountyAmount || 0
  );
  const newTaskVersionMinStake = getCellFromBaseUnits(
  ...
  const newTaskVersionDetails = {
    ...
    topStake: getCellFromBaseUnits(newTaskVersionTopStake),
    bounty: newTaskVersionTotalBountyInCell,
