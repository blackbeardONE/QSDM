...
import { useTaskWalletBalance } from '../../hooks/useTaskWalletBalance';
import { isNetworkingTask } from '../../utils';
import { getTaskBalanceEligibility } from '../../utils/taskBalanceEligibility';
...
function AvailableTaskRow({ task, columnsLayout, isPrivate }: Props) {
  ...
  const isQsdmMinerSystemTask =
    QSDM_TASK_RUNTIME_MODE === 'qsdm-native' &&
    isQsdmMinerSystemTaskId(publicKey);
  ...
  const deferredMinerBondAvailable = Boolean(
    isQsdmMinerSystemTask &&
      qsdmCoreStatus?.status?.mining?.deferred_bond_from_rewards
  );
  ...
  const accountBalanceToUse = accountBalance ?? 0;
  const hasResolvedTaskWalletBalance =
  ...
  const foreignQsdmStakeOwners = useMemo(() => {
    ...
    return Object.entries(task.stakeList || {})
      .filter(
        ([owner, stake]) =>
          owner.toLowerCase() !== qsdmSigner.toLowerCase() && Number(stake) > 0
      )
      .map(([owner]) => owner);
  ...
  const details: TaskStatsDataType = useMemo(
    () => ({
      ...
      bounty: getCellFromBaseUnits(
        isQsdmNativeSystemTask ? task.totalBountyAmount : totalBounty
      ),
  ...
  const kplBalance =
    kplBalanceListQueryData?.find((balance) => balance.mint === task.tokenType)
      ?.balance || 0;
  const { isKplTask, hasEnoughBalanceForStaking, hasEnoughNativeTokens } =
    getTaskBalanceEligibility({
      taskType: task.taskType,
      nativeBalance: accountBalanceToUse,
      tokenBalance: kplBalance,
      stakeAmount: valueToStake,
      existingStake: alreadyStakedTokensAmount,
      nativeFeeReserve: CRITICAL_MAIN_ACCOUNT_BALANCE,
      waiveNativeFeeReserve: deferredMinerBondAvailable,
    });
  ...
  const validateTask = useCallback(() => {
    ...
    const getErrorMessage = () => {
      ...
      const conditions = [
        {
          condition: hasResolvedTaskWalletBalance,
          errorMessage: usesNativeQsdmWallet
            ? 'load the active QSDM signer balance'
            : 'load your main account balance',
        },
        {
          condition:
            !hasResolvedTaskWalletBalance || hasEnoughBalanceForStaking,
          errorMessage: `have enough ${tokenTicker} to stake`,
          action: isKplTask ? undefined : showFundModal,
        },
        {
          condition: !hasResolvedTaskWalletBalance || hasEnoughNativeTokens,
  ...
  const showCreateMissingKplStakingKeyModalMemo = useCallback(
    ...
    [showConfirmRunTaskModal, showCreateMissingKplStakingKeyModal]
  );
