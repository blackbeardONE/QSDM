...
import { getTaskBalanceEligibility } from 'renderer/features/tasks/utils/taskBalanceEligibility';
...
export function AddPrivateTask({ columnsLayout, onClose }: Props) {
  ...
  const {
    ...
    walletAddress: taskWalletAddress,
  ...
  const minStakeToDisplay =
    task?.taskType === 'KPL'
      ...
      : getCellFromBaseUnits(task?.minimumStakeAmount || 0);
  ...
  const hasEnoughCell =
    (alreadyStakedTokensAmount &&
      accountBalanceToUse >= CRITICAL_MAIN_ACCOUNT_BALANCE &&
      alreadyStakedTokensAmount >= Number(minStake)) ||
    accountBalanceToUse >= valueToStake + CRITICAL_MAIN_ACCOUNT_BALANCE;
    ...
    <div className="border-t-2 border-white">
      <div className="pb-2 rounded-b-lg bg-[#8989C7]/10 hover:bg-[#8989C7]/[0.15] transition-all duration-300 ease-in-out">
        ...
        >
          ...
          <div className="mt-4">
            <EditStakeInput
              meetsMinimumStake={meetsMinimumStake && hasEnoughCell}
              stake={valueToStake}
