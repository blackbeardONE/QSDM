...
const formatRemainingTime = (seconds: number) => {
  ...
  return `${minutes}m ${remainingSeconds.toString().padStart(2, '0')}s left`;
};
...
const getRoundClock = ({
  ...
  const safeSlot = Math.max(0, Math.floor(currentSlot));
  const safeStartingSlot = Math.max(0, Math.floor(startingSlot));
  const safeRoundTime = Math.max(1, Math.floor(roundTime));
  const blocksSinceStart = Math.max(0, safeSlot - safeStartingSlot);
  const currentRound = Math.floor(blocksSinceStart / safeRoundTime);
  const blocksIntoRound = blocksSinceStart % safeRoundTime;
  const roundEndSlot = safeStartingSlot + (currentRound + 1) * safeRoundTime;
  const blocksRemaining = Math.max(0, roundEndSlot - safeSlot);

  return {
    currentRound,
    roundEndSlot,
    blocksRemaining,
    progressPercentage: (blocksIntoRound / safeRoundTime) * 100,
    estimatedSecondsRemaining: averageSlotTime
      ? (blocksRemaining * averageSlotTime) / 1000
      : 0,
  };
};
...
export function RewardsProgressBar({
  ...
  const {
    data: { displayedEarnings, progressPercentage, timeRemaining },
  ...
  const currentRound = roundClock?.currentRound || 0;
  const shouldDisplayRoundStatus = nextReward <= 0;
  const displayedRoundTimeRemaining =
    timeRemaining || roundClock?.estimatedSecondsRemaining || 0;
  const roundStatusLabel = roundClock
    ? `Round ${currentRound} · h ${currentSlot}/${roundClock.roundEndSlot} · ${formatRemainingTime(
        displayedRoundTimeRemaining
      )}`
    : `Round ${currentRound} · ${formatRemainingTime(
        displayedRoundTimeRemaining
      )}`;
    ...
    <Popover
      ...
      tooltipContent={
        <div className="space-y-2 text-left">
          {shouldDisplayRoundStatus ? (
            <div>
              <p>
                QSDM rounds are based on validator block height. The bar shows
                the current round window and resets when the next round begins.
              </p>
              <p className="mt-2">
                Current height {currentSlot}; round ends at height{' '}
                {roundClock?.roundEndSlot || 'unknown'}.
              </p>
              <p className="mt-2">
                {roundStatusLabel}
              </p>
            ...
            <Progress.Indicator
              ...
              style={{
                transform: `translateX(-${
                  100 -
                  (shouldDisplayRoundStatus && roundClock
                    ? roundClock.progressPercentage
