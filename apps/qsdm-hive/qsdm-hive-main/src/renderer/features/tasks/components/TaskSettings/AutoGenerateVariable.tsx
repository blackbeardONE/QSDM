...
export function AutoGenerateVariable({
  ...
  const claimStarterCell = async () => {
    await window.main.claimQsdmCellFaucet();
      ...
      const errorMessage = (t: Toast) =>
        hasToVerifyEmail ? (
          <div>
            ...
            This extension needs starter CELL in your QSDM wallet. Click{' '}
            <button
              className="font-bold"
              onClick={() => {
                claimStarterCell();
  ...
  const buttonText = hasToVerifyEmail ? 'Claim CELL' : 'Get Key';
