...
export function MyNodeTaskRow({
  ...
  const myStakeInCell = taskStake && getCellFromBaseUnits(taskStake);
  ...
  const details: TaskStatsDataType = {
    ...
    myStake: myStakeInCell,
    topStake: getCellFromBaseUnits(topStake),
    bounty: getCellFromBaseUnits(totalBounty),
    totalStakeInCell: getCellFromBaseUnits(totalStake),
  ...
  const toggleTaskButtonClasses = `group-hover/play:text-finnieTeal-100 group-hover/play:scale-110 transition-all duration-300 ease-in-out ${
    isRunning && myStakeInCell && myStakeInCell > 0
      ? 'text-finniePurple'
      : 'text-white'
  }
        ...
        <>
          ...
          {upgradeStatus === UpgradeStatus.NEW_VERSION_BEING_AUDITED ? (
            ...
            <>
              ...
              <RewardsCell
                nextRewardLabel={
                  isQsdmMinerSystemTask ? 'Hive Bounty' : undefined
                }
                pendingRewardsLabel={
                  isQsdmMinerSystemTask ? 'Pending' : undefined
                }
                allTimeRewardsLabel={
                  isQsdmMinerSystemTask ? 'Mined' : undefined
                }
                nextRewardTooltip={
