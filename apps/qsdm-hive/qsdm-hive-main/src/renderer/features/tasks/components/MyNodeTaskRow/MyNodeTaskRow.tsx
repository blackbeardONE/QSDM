import { Icon, PauseFill, PlayFill } from 'vendor/qsdm-styleguide';
...
import { getBestTooltipPosition, getCellFromBaseUnits } from 'utils';
...
const readSkyFangStakeCell = (status?: unknown) => {
  if (!status || typeof status !== 'object') return 0;
  const statusRecord = status as Record<string, unknown>;

  const value = [
    statusRecord.skyfang_stake_cell,
    statusRecord.skyFangStakeCell,
    statusRecord.in_game_stake_cell,
    statusRecord.inGameStakeCell,
    statusRecord.game_stake_cell,
    statusRecord.gameStakeCell,
    statusRecord.total_game_stake_cell,
    statusRecord.totalGameStakeCell,
  ].find((candidate) => {
...
export function MyNodeTaskRow({
  ...
  const isQsdmSystemTask = isQsdmSystemTaskId(publicKey);
  const isQsdmMinerSystemTask = isQsdmMinerSystemTaskId(publicKey);
  const isQsdmSkyFangLinkSystemTask =
    isQsdmSkyFangLinkSystemTaskId(publicKey);
  ...
  const { data: qsdmCoreStatus } = useQuery(
    [QueryKeys.QsdmCoreStatus, 'my-node-task-row'],
    getQsdmCoreStatus,
    {
  ...
  const { data: skyFangLinkStatus } = useQuery(
    [QueryKeys.QsdmSkyFangLinkStatus, publicKey, 'my-node-row'],
    getQsdmSkyFangLinkStatus,
    {
      enabled: isQsdmSkyFangLinkSystemTask,
      refetchInterval: 10000,
      staleTime: 5000,
    }
  );
  const qsdmTaskSigner = qsdmCoreStatus?.taskSigner?.sender || '';
  const correspondingStakingKey = useMemo(
    () => {
      if (QSDM_TASK_RUNTIME_MODE === 'qsdm-native' && task.taskType === 'CELL') {
        return qsdmTaskSigner || stakingAccountPublicKey;
      }

      return task.taskType === 'KPL'
  ...
  const myStakeInCell = taskStake ? getCellFromBaseUnits(taskStake) : 0;
  const skyFangStakeInCell = isQsdmSkyFangLinkSystemTask
    ? readSkyFangStakeCell(skyFangLinkStatus)
    : 0;
  const combinedSkyFangStakeInCell =
    myStakeInCell + skyFangStakeInCell;

  const isCoolingDown = useMemo(
    () => !canUnstake && !!myStakeInCell,
    [canUnstake, myStakeInCell]
  );
  ...
  const { data: queriedClaimedRewards = 0 } = useQuery(
    ...
    {
      enabled: !!task.publicKey,
      refetchInterval: isQsdmSystemTask ? 10000 : false,
      staleTime: isQsdmSystemTask ? 5000 : Infinity,
      onError: (error) => {
        console.warn('Unable to load all-time rewards for task', error);
      },
    }
  );

  useEffect(() => {
    setClaimedRewards(queriedClaimedRewards || 0);
    const pendingRewards =
      TaskService.getPendingRewardsByTask(task, correspondingStakingKey) || 0;
    setPendingRewards(pendingRewards);
  }, [queriedClaimedRewards, task, correspondingStakingKey]);
  ...
  const qsdmSystemStakeStats = useMemo(() => {
    const stakes = Object.values(task.stakeList || {})
      .map((stake) => Number(stake) || 0)
  ...
  const details: TaskStatsDataType = {
    nodesNumber: isQsdmSystemTask ? qsdmSystemStakeStats.totalNodes : totalNodes,
    minStake: getCellFromBaseUnits(task.minimumStakeAmount),
    myStake: isQsdmSkyFangLinkSystemTask
      ? combinedSkyFangStakeInCell
      : myStakeInCell,
    hiveStakeInCell: isQsdmSkyFangLinkSystemTask
      ? myStakeInCell
      : undefined,
    skyFangStakeInCell: isQsdmSkyFangLinkSystemTask
      ? skyFangStakeInCell
      : undefined,
    combinedStakeInCell: isQsdmSkyFangLinkSystemTask
      ? combinedSkyFangStakeInCell
      : undefined,
    topStake: getCellFromBaseUnits(
      isQsdmSystemTask ? qsdmSystemStakeStats.topStake : topStake
    ),
    bounty: getCellFromBaseUnits(
      isQsdmSystemTask ? task.totalBountyAmount : totalBounty
    ),
    totalStakeInCell: getCellFromBaseUnits(
      isQsdmSystemTask ? qsdmSystemStakeStats.totalStake : totalStake
    ),
    roundTime: task.roundTime,
  };

  const allTimeRewards = claimedRewards + pendingRewards;
  const allTimeRewardsInCell = getCellFromBaseUnits(allTimeRewards);
  const pendingRewardsInCell = getCellFromBaseUnits(pendingRewards);
  const displayIsRunning = isRunning;
  const displayTaskStatus = taskStatus;
  const nextRewardLabel = isQsdmMinerSystemTask
    ? 'Hive Bounty'
    : isQsdmSkyFangLinkSystemTask
    ? 'Stake-based'
    : undefined;
  const pendingRewardsLabel =
    isQsdmMinerSystemTask || isQsdmSkyFangLinkSystemTask
      ? 'Pending'
      : undefined;
  const allTimeRewardsLabel = isQsdmMinerSystemTask
    ? 'Mined'
    : isQsdmSkyFangLinkSystemTask
    ? 'Earned'
    : undefined;
  const nextRewardTooltip = isQsdmMinerSystemTask
    ? 'The miner task has no separate Hive-funded bounty. Mining payouts come from QSDM protocol emission when accepted proofs share a block.'
    : isQsdmSkyFangLinkSystemTask
    ? 'Sky Fang Link submits one reward proof per QSDM round while the active wallet remains linked. Rewards scale from Hive stake plus Sky Fang stake when reported.'
    : undefined;
  const pendingRewardsTooltip = isQsdmMinerSystemTask
    ? 'There are no claimable Hive bounty rewards for the miner task.'
    : isQsdmSkyFangLinkSystemTask
    ? 'Any Sky Fang Link reward waiting to be claimed from QSDM Core.'
    : undefined;
  const allTimeRewardsTooltip = isQsdmMinerSystemTask
    ? 'Protocol mining emission detected for the configured miner reward address since Hive started tracking it.'
    : isQsdmSkyFangLinkSystemTask
    ? 'Sky Fang Link rewards claimed by this QSDM wallet across completed rounds.'
    : undefined;
  ...
  const { showModal: showConfirmRunTaskModal } = useConfirmRunTask({
    ...
    onConfirm: async () => {
      try {
        ...
        const message = getErrorToDisplay(
          error as Error,
          ErrorContext.START_TASK
        );
        toast.error(message || 'Task failed to start');
      } finally {
        setLoading(false);
      }
    },
  ...
  const minStake = getCellFromBaseUnits(task.minimumStakeAmount);

  const isTaskDelisted = !task.isActive;
  const isTaskNotRunning = !isRunning;
  const hasNoStake = !myStakeInCell || myStakeInCell === 0;
  ...
  const taskVariablesRequired = isQsdmSystemTask
    ? []
    : metadata?.requirementsTags.filter(
  ...
  const canonicalChainBlocked =
    QSDM_TASK_RUNTIME_MODE === 'qsdm-native' &&
    qsdmCoreStatus !== undefined &&
    !qsdmCoreStatus.canonicalSafety?.safe;
  const isPlayPauseButtonDisabled =
    canonicalChainBlocked ||
    (isTaskNotRunning &&
      (hasNoStake ||
  ...
  const toggleTaskButtonClasses = `group-hover/play:text-finnieTeal-100 group-hover/play:scale-110 transition-all duration-300 ease-in-out ${
    displayIsRunning && myStakeInCell && myStakeInCell > 0
      ? 'text-finniePurple'
      : 'text-white'
  }
${isPlayPauseButtonDisabled && 'opacity-60'}`;
  ...
  const taskInfoProps = {
    ...
    shouldDisplayArchiveButton:
      accordionView === 'info' && !isRunning && !myStakeInCell,
    tokenTicker: kplToken?.symbol || NATIVE_TOKEN_SYMBOL,
  };
          ...
          {upgradeStatus === UpgradeStatus.NEW_VERSION_BEING_AUDITED ? (
            ...
            <>
              ...
              <StatusBar
                ...
                tokenSymbol={kplToken?.symbol || NATIVE_TOKEN_SYMBOL}
                pendingRewards={pendingRewardsInCell}
                taskName={taskName}
                isPrivate={isPrivate}
                isBonusTask={isBonusTask}
                isBountyEmpty={isBountyEmpty}
              ...
              <RewardsCell
                ...
                pendingRewardsLabel={pendingRewardsLabel}
                allTimeRewardsLabel={allTimeRewardsLabel}
                nextRewardTooltip={nextRewardTooltip}
                pendingRewardsTooltip={pendingRewardsTooltip}
                allTimeRewardsTooltip={allTimeRewardsTooltip}
                nextReward={
                  isQsdmMinerSystemTask ? (
                    ...
                    <CurrencyDisplay
                      amount={lastReward}
                      currency={kplToken?.symbol || NATIVE_TOKEN_SYMBOL}
                      precision={2}
                    />
                  ) : (
                    'N/A'
                ...
                pendingRewards={
                  <CurrencyDisplay
                    currency={kplToken?.symbol || NATIVE_TOKEN_SYMBOL}
                    amount={pendingRewardsInCell}
                    precision={
                      pendingRewardsInCell > 10 || pendingRewardsInCell === 0
                        ? 0
                        : 3
                    }
                    hideSymbol
                ...
                allTimeRewards={
                  <CurrencyDisplay
                    currency={kplToken?.symbol || NATIVE_TOKEN_SYMBOL}
                    amount={allTimeRewardsInCell}
                    precision={
                      allTimeRewardsInCell > 10 || allTimeRewardsInCell === 0
                        ? 0
                        : 2
                    }
                    hideSymbol
                ...
                {loading ? (
                  ...
                  >
                    <Button
                      ...
                      icon={
                        <Icon
                          source={displayIsRunning ? PauseFill : PlayFill}
                          size={20}
