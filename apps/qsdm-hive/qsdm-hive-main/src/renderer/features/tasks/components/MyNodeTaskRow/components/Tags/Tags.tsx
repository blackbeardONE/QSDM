...
export function Tags({ isLoadingTags, tags, isPrivate }: Props) {
  const formattedTags = tags.filter((tag) => tag !== 'ORCA_TASK').map((tag) => {
    if (TAGS[tag]) {
      return TAGS[tag];
    }
  ...
  return (
    <div className="min-w-[85px] flex flex-col gap-2 my-auto cursor-default">
      ...
      <Popover
        ...
        tooltipContent={
          isPrivate
            ? 'This task has not been verified by the QSDM Hive team. Run with caution.'
            : 'The QSDM Hive team has verified this task and it is safe to run.'
        }
      >
        <div
