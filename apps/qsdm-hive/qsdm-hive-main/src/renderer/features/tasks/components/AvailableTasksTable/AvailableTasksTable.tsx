...
import { isQsdmMotherHiveSystemTaskId } from 'config/qsdmSystemTasks';
import {
...
export function AvailableTasksTable() {
  ...
  const shouldDisplayQsdmEcosystemCheckbox =
    !!brandingConfig?.organizationKPLTokenAddress;
  const shouldDisplayBroaderQsdmEcosystemTasks =
    userConfig?.shouldDisplayBroaderQsdmEcosystemTasks || false;
  ...
  const toggleDisplayQsdmEcosystemCheckbox = () => {
    userConfigMutation.mutate({
      settings: {
        shouldDisplayBroaderQsdmEcosystemTasks:
          !shouldDisplayBroaderQsdmEcosystemTasks,
      },
    });
  };
        ...
        <div className="absolute -top-20 left-52 flex flex-col justify-between items-start gap-1 mb-4 w-fit">
          ...
          <div className="flex justify-start items-start mb-4 w-fit -ml-7 mt-0">
            ...
            {shouldDisplayQsdmEcosystemCheckbox && (
              ...
              >
                <Switch
                  id="qsdm-ecosystem-tasks"
                  isChecked={shouldDisplayBroaderQsdmEcosystemTasks}
                  onSwitch={toggleDisplayQsdmEcosystemCheckbox}
                  label="Show other QSDM Hive ecosystem tasks"
                />
              </motion.div>
            )}
          </div>
            ...
            {thereAreNoTasks && (
              <div className="flex items-center justify-center w-full h-full text-white">
                <div className="w-[363px] h-[363px] flex flex-col justify-center items-center">
                  <div
                    aria-hidden="true"
                    className="relative flex items-center justify-center w-24 h-24 rounded-full border border-finnieTeal bg-finnieBlue-light-tertiary"
                  >
                    <div className="absolute w-16 h-16 border rounded-full border-finnieTeal/40" />
                    <div className="absolute w-10 h-10 border rounded-full border-green-2/50" />
                    <span className="text-3xl font-semibold text-green-2">
                      +
                    </span>
                  </div>
