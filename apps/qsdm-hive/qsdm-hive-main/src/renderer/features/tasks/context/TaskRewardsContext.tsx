...
const QSDM_NATIVE_SLOT_REFETCH_INTERVAL = 10 * 1000;
const LEGACY_SLOT_REFETCH_INTERVAL = 5 * 60 * 1000;
...
export function TaskRewardsProvider({
  ...
  const currentSlotRefetchInterval = isQsdmNative
    ? QSDM_NATIVE_SLOT_REFETCH_INTERVAL
    : LEGACY_SLOT_REFETCH_INTERVAL;

  const { data: currentSlot } = useQuery({
    queryKey: [QueryKeys.CurrentSlot],
    queryFn: getCurrentSlot,
    refetchInterval: currentSlotRefetchInterval,
    refetchIntervalInBackground: isQsdmNative,
    staleTime: currentSlotRefetchInterval,
