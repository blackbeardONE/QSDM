...
function RunFirstTask() {
  ...
  const totalToStakeInCELL = useMemo(
    () => getCellFromBaseUnits(totalToStake),
    [totalToStake]
  );
  ...
  const tasksFeeInCell = getCellFromBaseUnits(tasksFee);

  const stakeAmount =
    alreadyStakedTokensAmount >= selectedTasks?.[0]?.minStake
      ? 0
      : totalToStakeInCELL;
  const totalCellToUse = stakeAmount + tasksFeeInCell;
  const { showNotEnoughFunds } = useNotEnoughFunds();

  const totalStakeInCell = useMemo(
    () =>
      tasksToRun.reduce(
        (acc, task) =>
          task.taskType === 'CELL' ? acc + getCellFromBaseUnits(task.stake) : acc,
        0
      ),
    [tasksToRun]
  );
  ...
  const totalStakeInKpl = useMemo(
    () =>
      tasksToRun.reduce(
        (acc, task) =>
          task.taskType === 'KPL' ? acc + getCellFromBaseUnits(task.stake) : acc,
        0
      ),
    [tasksToRun]
  );

  const totalFeesInCell = useMemo(
    () =>
      tasksToRun.reduce(
        (acc, task) => acc + getCellFromBaseUnits(config.node.TASK_FEE),
        0
      ),
    [tasksToRun]
  );
  ...
  const kplBalanceToDisplay = getCellFromBaseUnits(kplBalance);
  const balanceInCell = getCellFromBaseUnits(mainAccountBalance);

  const balanceToDisplay =
    selectedTasks?.[0]?.taskType === 'CELL'
      ? balanceInCell
      : kplBalanceToDisplay;

  const hasEnoughForStake = useMemo(() => {
    const hasEnoughCell = balanceInCell >= totalStakeInCell + totalFeesInCell;
    const hasEnoughKpl = kplBalance >= totalStakeInKpl;
    return isKPlTask ? hasEnoughKpl : hasEnoughCell;
  }, [
    balanceInCell,
    kplBalance,
    totalStakeInCell,
      ...
      <div className="flex flex-col items-center justify-evenly w-full h-full text-center">
        <div>
          ...
          <div className="text-sm mb-5 text-center">
            You&apos;ll need some {NATIVE_TOKEN_SYMBOL} to run your first task,
            to pay for gas fees. Just press{' '}
            <span className="font-semibold">Confirm</span> to start running.{' '}
            <br /> The more tasks you complete, the more you earn.
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between w-full px-4 mt-1 md2h:mt-3 text-sm font-semibold leading-5">
                    ...
                    {!!alreadyStakedTokensAmount && (
                      <div className="flex gap-2">
                        ...
                        <p>
                          {getCellFromBaseUnits(alreadyStakedTokensAmount)}{' '}
                          {tokenTicker}
                        </p>
                      </div>
                    )}
        ...
        <div className="flex flex-col items-center">
          ...
          {showMinimumStakeError ? (
            <p className="pt-3.5 text-xs text-finnieRed">
              Whoops! Make sure you stake at least {minimumStake}{' '}
