...
function ConfirmYourStake() {
  ...
  const totalCELLStaked = useMemo(
    () => getCellFromBaseUnits(sum(tasksToRun.map((task) => task.stake))),
    [tasksToRun]
  );
  const tasksFee = TASK_FEE * tasksToRun.length;
  const tasksFeeInCell = getCellFromBaseUnits(tasksFee);
  const totalCellToUse = totalCELLStaked + tasksFeeInCell;

  const handleConfirm = () => {
    if (balanceInCell < totalCellToUse) {
      showNotEnoughFunds();
    } else {
      runAllTasks();
    }
