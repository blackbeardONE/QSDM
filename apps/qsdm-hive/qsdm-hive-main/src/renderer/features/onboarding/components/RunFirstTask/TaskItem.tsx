...
function TaskItem({
  ...
  const minStakeInCell = getCellFromBaseUnits(task.minimumStakeAmount);
  const stakeValueInCELL = getCellFromBaseUnits(stakeValue);
  ...
  const details = {
    ...
    topStake: getCellFromBaseUnits(taskInfo.topStake ?? 0),
    bounty: getCellFromBaseUnits(taskInfo?.totalBounty ?? 0),
    totalStakeInCell: getCellFromBaseUnits(taskInfo?.totalStake ?? 0),
  };
        ...
        <div className="grid w-full text-left rounded-md bg-finnieBlue-light-secondary h-13 grid-cols-first-task place-content-center cursor-pointer">
          ...
          <div className="col-span-6 my-auto cursor-pointer">
            <Popover
              ...
              tooltipContent={
                <p>
                  This task was created by the QSDM Hive team and is safe to run.
                </p>
              }
            >
              {task.taskName}
