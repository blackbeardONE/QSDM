...
export const onboardingSteps = [
  ...
  {
    label: 'Back up and fund your QSDM wallet.',
...
function OnboardingLayout({ children }: PropsType) {
  ...
  const { handleSaveUserAppConfig, userConfig, isUserConfigLoading } =
    useUserAppConfig({
      onConfigSaveSuccess: () => {
  ...
  const hasCompletedOnboarding =
    ...
    (!!userConfig?.pin && !!mainAccountPubKey);
  const isInitialScreen = currentPath === AppRoute.OnboardingInitialScreen;
  const hasLocalAccountWithoutPin = !!mainAccountPubKey && !userConfig?.pin;

  useEffect(() => {
    if (!isUserConfigLoading && hasCompletedOnboarding) {
      navigate(AppRoute.AppInit, {
        replace: true,
    ...
    if (!isUserConfigLoading && isInitialScreen && hasLocalAccountWithoutPin) {
  ...
  return isInitialScreen ? (
    ...
    <div className="qsdm-cell-screen flex flex-col h-full w-full text-white">
      <div className="relative z-10 w-full justify-between flex h-fit p-8">
      ...
      <div className="relative z-10 h-full w-3/4 max-w-[1200px] flex items-center justify-center mx-auto transition-all duration-300 ease-in-out">
        {children}
      </div>
      <div className="relative z-10 flex flex-col min-h-[50px]">
        {displaySkipButton && (
          <Button
