...
const getQsdmCoreStatusLabel = (status?: QsdmCoreStatusResponse) => {
  ...
  if (status.connectionState === 'offline' || !status.healthy) return 'Offline';
  if (status.canonicalSafety.usingGatewayFallback) return 'Gateway Fallback';
  if (status.coreConnectionMode === 'gateway') return 'Canonical Gateway';
  if (status.coreConnectionMode === 'custom') return 'Canonical Custom';
...
const getQsdmCoreStatusColor = (
  ...
  if (status.connectionState === 'degraded') return 'text-yellow-500';
  if (!status.healthy || !status.canonicalSafety?.safe) return 'text-red-500';
...
export function RPCWidget() {
  ...
  const qsdmCoreStatusLabel = getQsdmCoreStatusLabel(qsdmCoreStatus);
  const qsdmCoreStatusColor = getQsdmCoreStatusColor(qsdmCoreStatus, isVip);
  ...
  const getSignedLoopTitle = () => {
    if (!qsdmCoreStatus?.canonicalSafety?.safe) {
      return (
        qsdmCoreStatus?.canonicalSafety?.detail ||
        'The canonical QSDM chain must be verified'
      );
    }
  ...
  return (
    <div className="flex flex-row items-center gap-2 px-3 py-1.5 rounded-full bg-[#BEF0ED]/10 whitespace-nowrap">
      ...
      <button
        type="button"
        className={`text-sm font-medium ${qsdmSignerColor} ${
          qsdmSignerReady ? 'cursor-default' : 'underline hover:text-white'
        }`}
        disabled={qsdmSignerReady}
        onClick={() => navigate(AppRoute.SettingsWallet)}
        title={
          qsdmSignerReady
            ? 'QSDM signer wallet is ready'
            : 'Set up the QSDM signer wallet'
        }
      >
        {qsdmSignerLabel}
      </button>
