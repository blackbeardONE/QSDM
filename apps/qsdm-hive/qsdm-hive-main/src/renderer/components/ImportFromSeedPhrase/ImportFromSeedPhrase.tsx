...
function ImportFromSeedPhrase({
  ...
  const validateWords = async (phraseWords: string[]) => {
    const validationResults = await Promise.all(
      phraseWords.map((word) =>
        word ? window.main.validateBip39Word({ word }) : Promise.resolve(true)
      )
    );
    return validationResults.map((isValidWord) => !isValidWord);
  };

  const applyPhraseWords = async (phraseWords: string[]) => {
    setWords(phraseWords);
    setWordsErrors(await validateWords(phraseWords));
  };
  ...
  const handleInputChange = async (
    ...
    const pastedPhraseWords = normalizePhrase(value);

    if (pastedPhraseWords.length === 12) {
      await applyPhraseWords(pastedPhraseWords);
      return;
    }
    ...
    const isValidWord = word
      ? await window.main.validateBip39Word({ word })
      : true;
  ...
  const handlePaste = async (e: ClipboardEvent<HTMLInputElement>) => {
    const phraseWords = normalizePhrase(e.clipboardData.getData('text'));
    ...
    e.preventDefault();
    setError('');
    await applyPhraseWords(phraseWords);
  };
  ...
  const validateKeyPhrase = (words: string[]) => {
    ...
    const error = !allWordsAreProvided
      ? 'The Hive recovery phrase is not complete'
      : !allWordsAreValid
