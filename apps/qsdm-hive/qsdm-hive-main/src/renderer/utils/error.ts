...
export const getErrorToDisplay = (
  ...
  if (isDetailedError) {
    ...
    if (
      parsedError.type === ErrorType.TASK_START &&
      typeof parsedError.detailed === 'string' &&
      parsedError.detailed.trim()
    ) {
      return parsedError.detailed;
    }
...
> = {
  [ErrorType.NO_VALID_MNEMONIC]:
    'Please provide a valid Hive recovery phrase',
  ...
  [ErrorType.NO_MNEMONIC]:
    'Please provide a Hive recovery phrase to create a local Hive profile',
  [ErrorType.NO_JSON_KEY]: 'Please provide a Hive key file to import a profile',
  [ErrorType.DUPLICATE_ACCOUNT]:
    'A Hive profile with the same recovery phrase already exists',
  ...
  [ErrorType.NODE_INITIALIZATION_FAILED]:
    'Node initialization failed. Please restart QSDM Hive',
