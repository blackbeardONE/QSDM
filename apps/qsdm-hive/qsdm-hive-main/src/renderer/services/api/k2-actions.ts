...
export const stakeOnTask = (params: DelegateStakeParam) => {
  ...
  const stakeAmountInCELL = getCellFromBaseUnits(params.stakeAmount);
  const paramsWithStakeInCELL = {
    ...params,
    stakeAmount: stakeAmountInCELL,
  };
  return window.main.delegateStake(paramsWithStakeInCELL);
};
export const transferCellFromMainWallet = (
  accountName: string,
  amount: number,
  toWalletAddress: string
) => {
  return window.main.transferCellFromMainWallet({
    accountName,
    amount,
    toWalletAddress,
  });
};
export const transferCellFromStakingWallet = (
  accountName: string,
  amount: number,
  toWalletAddress: string
) => {
  return window.main.transferCellFromStakingWallet({
    accountName,
    amount,
    toWalletAddress,
  });
...
export const claimRewards = async (): Promise<void> => {
  ...
  const getPendingRewardsByTask = async (task?: Task) => {
    if (QSDM_TASK_RUNTIME_MODE === 'qsdm-native') {
      if (QSDM_TASK_ACTION_SENDER) {
        return task?.availableBalances[QSDM_TASK_ACTION_SENDER] || 0;
      }
      return Math.max(0, ...Object.values(task?.availableBalances || {}));
    }
  ...
  const promisesToClaimRewards = tasksWithClaimableRewards.map(async (task) => {
    ...
    const handlerToClaimReward = QSDM_TASK_RUNTIME_MODE === 'qsdm-native'
...
export const fundStakingWalletFromMainWallet = async (amountInBaseUnits: number) => {
  return window.main.creditStakingWalletFromMainWallet({ amountInBaseUnits });
};
